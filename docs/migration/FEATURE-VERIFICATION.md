...
| 浏览器控制 | skill/browser-plugin 在 | 应用内“用 open-science-browser 打开 example.com” | 🟡 2026-09-07 部分通过:浏览器窗口能弹出;但仍存在 MCP 代理层稳定性问题(库存报错/调用超时)。修复了三层根因:`\\?\` 前缀(EISDIR)、wrapper 可选凭据、缺省会话租约。剩余为代理并发/超时调试(dev 任务,暂不再请用户重试) |
