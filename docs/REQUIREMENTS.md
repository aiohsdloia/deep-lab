...
- **Status.** ✅ Every agent write appends a version record to
  `.openlab/provenance.jsonl` (code, tool, model, session, timestamp, and
  ...
  lockfile `.openlab/env/<hash>.txt`; the record carries `{count, hash}`, the
...
- **Status.** 🟡 One documented chart design system — a validated
  ...
  places (`@deeplab/shared` chartPalette, `index.css --series-*`,
  `openlab.mplstyle` applied by the `publication-figures` skill) — so agent
