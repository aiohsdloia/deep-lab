...
- Remote machines the user configured (SSH servers, GPU boxes, Slurm clusters)
  are listed in this workspace at `.openlab/compute.json` (the app keeps it
...
11. You CAN read images: when an image is shown to you (a pasted/shown/screenshot
    image, a workspace figure, a chart/table/photo), read it with the `image-tools`
    skill (`recognize.py describe|ocr|read`) — never say you cannot view images
    just because the main model lacks native vision.
