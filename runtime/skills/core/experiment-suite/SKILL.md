...
End-to-end experiment package builder. **Single stage, full quality from the
start.** A DeepLab dsh agent writes and executes the package through dsh
filesystem, shell, web/MCP, job, and approval tools. This skill contains
procedure, reference playbooks, and figure-example scripts, with no LLM SDK.

## DeepLab / dsh execution contract

- Generic Write, Read, Bash, WebSearch, and WebFetch names in the references
  map to dsh filesystem, platform-shell, and web/MCP tools. PowerShell is used
  on Windows and Bash elsewhere.
- Use dsh background jobs only for genuinely long commands, and collect their
  terminal status before recording a result as measured.
- Keep code, inputs, outputs, and reports inside the workspace. Never modify
  `.deeplab/` or `.dsh/`; DeepLab records runs and provenance from dsh events.
- Translate shell snippets to the active platform. If symlink creation is not
  available, use a normal `latest` directory.
- Never relabel simulated or illustrative numbers as measured. A command must
  have actually completed and its output must be inspected before a result is
  marked measured.
