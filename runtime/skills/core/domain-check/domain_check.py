...
# --------------------------------------------------------------------------- #
# Phylogenetics — distances & branch support
# --------------------------------------------------------------------------- #

# Variable / call names that mark a value as an (unaligned) biological sequence.
_SEQ_NAMES = ("seq", "sequence", "dna", "rna", "protein", "proteome", "genome", "reads", "contig")
# Name fragments that mark a value as a genetic/evolutionary distance.
_EVOL_DIST = ("dist", "distance", "pdist", "dna_dist", "substitutions", "d_xy")
# Distance calls that silently assume Euclidean geometry — wrong on sequences.
_EUC_DIST_CALLS = ("pdist", "pairwise_distances", "distance", "dist", "cdist")
_ALIGNMENT = re.compile(
    r"\b(align|alignment|mafft|clustal|muscle|needle|water|kalign)\b", re.IGNORECASE
)
_BOOTSTRAP = re.compile(
    r"\b(bootstrap|boot_trees|ufboot|sh[- ]?alrt|ultrafast|alrt)\b", re.IGNORECASE
)


def _seq_distance(ctx: Ctx) -> list[Finding]:
    """Euclidean / squared-diff / pdist on sequence-like operands, with no
    alignment and no substitution-model mention anywhere in the file."""
    out: list[Finding] = []
    src = ctx.src
    if _ALIGNMENT.search(src):
        return out  # an alignment is a legitimate precondition — stay silent
    if re.search(r"models?\.(jc69|jc|k80|k2p|gtr|hky|tn93)|dist\.dna|dna\.dist", src, re.IGNORECASE):
        return out
    if ctx.tree is not None:
        for node in ast.walk(ctx.tree):
            is_euc = _is_sq_diff(node) or (
                isinstance(node, ast.Call) and _call_name(node) in _EUC_DIST_CALLS
            )
            if is_euc and _has_name_like(node, *_SEQ_NAMES):
                out.append(Finding(
                    "error", "phylo · distance",
                    "Euclidean distance applied to unaligned sequences",
                    ctx.snippet(ctx.line_of(node))
                    + "\n  raw sequences are not numeric vectors — a distance "
                    "matrix from them must come from an alignment plus a "
                    "substitution model, not pdist / (a-b)**2.",
                ))
    else:  # R fallback
        for m in re.finditer(r"\bpdist\s*\(\s*[^)]*(?:seq|dna|protein)[^)]*\)", src, re.IGNORECASE):
            ln = src[: m.start()].count("\n") + 1
            out.append(Finding(
                "error", "phylo · distance",
                "Euclidean distance applied to unaligned sequences",
                ctx.snippet(ln) + f"\n  `{m.group(0).strip()}` — see above.",
            ))
    return out


def _tree_support(ctx: Ctx) -> list[Finding]:
    """An inference + newick/nexus write with no bootstrap/SH-aLRT anywhere: the
    tree is reported without branch-support values."""
    out: list[Finding] = []
    src = ctx.src
    inferred = re.search(r"\b(infer\w*|build\w*|estimate\w*|parsimony|nj\b|neighbor|ml\b|iq[\s-]?tree)\b", src, re.IGNORECASE)
    writes = re.search(r"\.write\s*\(|Phylo\.write|save\s*\([^)]*(?:newick|nexus)|tree\.newick|tre\|", src, re.IGNORECASE)
    if not (inferred and writes):
        return out
    if _BOOTSTRAP.search(src):
        return out
    m = writes
    ln = src[: m.start()].count("\n") + 1
    out.append(Finding(
        "warn", "phylo · support",
        "Tree inferred and written with no branch-support values",
        ctx.snippet(ln)
        + "\n  the file infers a tree and writes it, but never computes "
        "bootstrap / UFBoot / SH-aLRT. ML trees reported without support cannot "
        "be distinguished from a parsimony topology.",
    ))
    return out


def check_phylo(ctx: Ctx) -> list[Finding]:
    return _seq_distance(ctx) + _tree_support(ctx)


# --------------------------------------------------------------------------- #
# Ecology — composition & categorical reductions
# --------------------------------------------------------------------------- #

# Nominal codes common in community ecology that must never be averaged.
_ECO_NOMINAL = ("code", "site_id", "plot", "habitat", "spp", "species", "community", "guild", "treatment")
_REDUCTION = re.compile(r"\.(mean|median|std|var|sum)\s*\(", re.IGNORECASE)
_COMPOSITIONAL = re.compile(r"\b(bray|jaccard|vegdist|relat|abund|prop|compos|clr|ilr|aitchison)\b", re.IGNORECASE)


def check_ecology(ctx: Ctx) -> list[Finding]:
    out: list[Finding] = []
    src = ctx.src
    # (1) Numeric reduction directly on a categorical site/community code.
    if ctx.tree is not None:
        for node in ast.walk(ctx.tree):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute):
                if node.func.attr not in {"mean", "median", "std", "var"}:
                    continue
                # A groupby(key).mean() is the CORRECT reduction — never flag it.
                if any(isinstance(x, ast.Call) and _call_name(x) == "groupby" for x in ast.walk(node)):
                    continue
                if _has_name_like(node, *_ECO_NOMINAL):
                    out.append(Finding(
                        "error", "ecology · categorical",
                        "Numeric reduction on a categorical code",
                        ctx.snippet(ctx.line_of(node))
                        + "\n  averaging a site / habitat / species code treats an "
                        "unordered label as an interval quantity.",
                    ))
    # (2) Euclidean distance on a community composition matrix.
    if not _COMPOSITIONAL.search(src):
        for m in re.finditer(
            r"\b(?:pdist|pairwise_distances|vegdist|dist)\s*\([^)]*\)", src, re.IGNORECASE
        ):
            if not re.search(r"(counts|matrix|comm|spp|abund|community|abundance)", m.group(0), re.IGNORECASE):
                continue
            ln = src[: m.start()].count("\n") + 1
            out.append(Finding(
                "warn", "ecology · composition",
                "Euclidean distance on a community matrix",
                ctx.snippet(ln)
                + "\n  community composition is compositional — Euclidean "
                "distance is invalid; use Bray-Curtis / Jaccard / Aitchison.",
            ))
    return out


# --------------------------------------------------------------------------- #
# Evolution — substitution models
# --------------------------------------------------------------------------- #

# Jukes-Cantor (and relatives) always contain these three tokens together.
_JC_FORMULA = re.compile(
    r"-?\s*(?:3\s*/\s*4|0?\.75)\s*\*\s*(?:np\.)?(?:log|ln)\s*\([^)]*\)",
    re.IGNORECASE,
)
_CLIP = re.compile(r"\b(np\.)?(clip|minimum|maximum|min|max|where)\s*\(", re.IGNORECASE)


def check_evolution(ctx: Ctx) -> list[Finding]:
    out: list[Finding] = []
    src = ctx.src
    # (1) JC69-style correction computed without guarding p → 0.75.
    m = _JC_FORMULA.search(src)
    if m and not _CLIP.search(src):
        ln = src[: m.start()].count("\n") + 1
        out.append(Finding(
            "error", "evolution · jukes-cantor",
            "Jukes-Cantor correction with no p ≤ 0.75 guard",
            ctx.snippet(ln)
            + "\n  the term `1 - 4p/3` goes negative (log → NaN) when p > 0.75; "
            "clip the observed proportion before correcting.",
        ))
    # (2) A naive p-distance returned as an evolutionary rate.
    if not re.search(r"\b(jc69|jc|k80|k2p|gtr|hky|tn93|model)\b", src, re.IGNORECASE):
        for m in re.finditer(
            r"\b(?:substitution|evolutionary)?\s*dist(?:ance)?\s*=.*?/\s*(?:n\w*|l\w*|sites?)\b",
            src, re.IGNORECASE,
        ):
            ln = src[: m.start()].count("\n") + 1
            out.append(Finding(
                "warn", "evolution · rate",
                "Naive p-distance reported as an evolutionary rate",
                ctx.snippet(ln)
                + "\n  raw differences per site under-estimate divergence once "
                "multiple hits accumulate — use a substitution model (JC69/K80/ML).",
            ))
    return out
...
def discover(root: Path) -> list[Path]:
    skip = {"node_modules", "__pycache__", ".git", ".openlab", ".openscience", ".venv", "venv"}
    out: list[Path] = []
