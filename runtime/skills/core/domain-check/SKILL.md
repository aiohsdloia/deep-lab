...
- **phylo · distance** — Euclidean / squared-diff / `pdist` applied to raw,
  unaligned sequences (treated as numeric vectors) with no alignment and no
  substitution model anywhere in the file. A distance matrix must come from an
  alignment plus a substitution model, not `(a-b)**2`.
- **phylo · support** — a tree is inferred *and* written (newick/nexus) but the
  file never computes bootstrap / UFBoot / SH-aLRT, so the reported tree carries
  no branch-support values and cannot be distinguished from a parsimony topology.
- **ecology · categorical** — `.mean()`/`.median()`/`.std()` on a nominal
  ecological code (`site_id`, `plot`, `habitat`, `spp`, …). A `groupby('plot')`
  key is correct usage and is not flagged.
- **ecology · composition** — Euclidean / `pdist` / `vegdist` on a community
  composition matrix with no Bray-Curtis / Jaccard / Aitchison (compositional)
  method mentioned — Euclidean geometry is invalid on compositional data.
- **evolution · jukes-cantor** — a JC69-style correction
  (`-3/4 · log(1 - 4p/3)`) computed without clipping p, which goes NaN once
  p exceeds 0.75.
- **evolution · rate** — a naive p-distance (differences per site) reported as an
  evolutionary distance/rate with no substitution-model mention — it
  under-estimates divergence once multiple hits accumulate.
