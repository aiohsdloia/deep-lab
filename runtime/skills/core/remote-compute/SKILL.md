...
1. `cat .openlab/compute.json` in the workspace (the app keeps this file in
   ...
   `{"machines":[{"host":"home-3090","label":"8x3090",
     ...
     "workdir":null,"os":"linux"}]}`
   `workdir`, when set, is the machine's default remote working directory — the
   base folder the job lands under. `os` (`"win"`/`"linux"` or null) is a manual
   OS override the probe uses; null means auto-detect. The directory is hidden —
...
1. Pick a job name and build the remote dir path under the machine's `workdir`
   (the machine's default working directory from compute.json) when one is set,
   else `openlab` under your SSH home (remember the literal string — shell
   variables do not survive between separate ssh calls):
   `WORKDIR=<machine workdir from compute.json, else "openlab">`
   `REMOTE=${WORKDIR}/jobs/<name>-<YYYYmmdd-HHMMSS>`
...
2. Submit:
   ```bash
   WORKDIR=<machine workdir from compute.json, else "openlab">
   REMOTE=${WORKDIR}/jobs/<job-name>-$(date +%Y%m%d-%H%M%S)
