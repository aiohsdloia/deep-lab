...
The style file `openlab.mplstyle` sits next to this SKILL.md. Load it by
...
STYLE = Path(__file__).resolve().parent / "openlab.mplstyle" if "__file__" in dir() else None
...
1. Style applied (palette + chrome from `openlab.mplstyle`).
