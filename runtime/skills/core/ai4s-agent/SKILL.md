...
Top-level entry point for the AI4S research stack. This skill contains **no
scientific stage of its own**. It coordinates four downstream skills through
DeepSeek Harness (dsh) subagents, keeps their paths and disclosures consistent,
and verifies each stage's files before starting a dependent stage.
...
Each downstream skill is **single-stage and self-sufficient**: a dedicated dsh
subagent loads that skill's `SKILL.md` and produces its complete artifact in the
shared workspace. There is no skeleton/enrichment split. The parent agent owns
coordination, path checks, user decisions, and disclosure consistency.

## DeepLab / dsh orchestration contract

- Use dsh's `subagent` tool for every stage. The spawn provider gives a child no
  parent conversation, so every prompt must state the topic, workspace-relative
  inputs, expected output root, and the skill name to load.
- Set `run_in_background: false` when the next step depends on a child's result.
  For a specific topic, start the literature-survey and experiment-suite calls
  together in one assistant turn; dsh runs sibling tool calls concurrently.
- A child report is not proof of completion. After every child returns, inspect
  the required files in the shared workspace. Re-run or repair the stage when
  its quality gate or required output is missing.
- Keep all writes inside the current workspace. Never write into `.deeplab/` or
  `.dsh/`; DeepLab and dsh own those directories.
- DSH uses PowerShell on Windows and Bash on macOS/Linux. Commands in downstream
  references are intent, not a required shell dialect. Translate them to the
  active shell and do not require symbolic-link privileges: when `latest` cannot
  be a symlink, use a normal `latest` directory.
...
- **Topic** ("Transformer-based long-horizon forecasting with patch tokenisation") — skip `research-explorer`; start `literature-survey` and `experiment-suite` in parallel, then run `paper-writer` after both outputs pass inspection.
...
Start one foreground dsh subagent with a standalone instruction to load the
`research-explorer` skill. Follow its 5 steps to produce:
...
Start a foreground dsh subagent with a standalone instruction to load the
`literature-survey` skill with `$TOPIC`. It produces:
...
Start a foreground dsh subagent with a standalone instruction to load the
`experiment-suite` skill with `$TOPIC`. When there is no dependency between the
survey and experiment stages, start both subagent calls in the same assistant
turn so dsh can run them concurrently. It produces:
...
Only after inspecting the survey and experiment outputs, start a foreground dsh
subagent with a standalone instruction to load the `paper-writer` skill with
`$TOPIC`. Its cross-skill conventions automatically pick up Steps 3 and 4:
...
- Re-runs the paper-writer freshness gate after expansion; adding older
  foundational references must not silently make a fast-moving bibliography
  stale.
...
- **Never collapse the four skills into one agent run.** Each stage runs in its
  own dsh subagent; each skill's `SKILL.md` remains the source of truth for what
  counts as done.
- Do not invoke an external agent CLI. DSH is the runner and owns child-agent
  lifecycle, cancellation, progress, and the shared workspace.
