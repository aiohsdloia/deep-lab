...
Brand name: **DeepLab** — "Local-first, model-agnostic AI research
workbench for macOS, Windows & Linux, powered by the DeepSeek Harness (dsh)
agent runtime." Bundle identifier is `com.sculab.deeplab` and internal
`@deeplab/*` package names; the data directory is `~/Documents/DeepLab` and
per-workspace app metadata lives in `<workspace>/.deeplab/`.
...
for macOS, Windows, and Linux. It reproduces the functionality of Open Lab
(open-lab/) but replaces the agent runtime with the **DeepSeek Harness (dsh)**:
the app spawns `dsh --profile web` as a bundled sidecar and talks to it through
the dsh `/api` HTTP+WebSocket gateway (the dsh client framework). See
`README.md`, `docs/PRD.md`, and `docs/TECHNICAL_DESIGN.md`.

Recommended stack: **Tauri 2 + React + TypeScript + Vite**, Tailwind + Radix UI,
**dsh** as the agent runtime (bundled Node sidecar via `runtime/dsh/launcher.mjs`;
HTTP POST unary + WebSocket event streams), local workspace + SQLite + JSONL
provenance.

## Repository map

- `apps/desktop/` — Tauri + React desktop shell (`src/` frontend, `src-tauri/` Rust).
- `packages/` — `ui`, `shared`, `sdk` (the `AgentRuntime` seam; `DshRuntime` is
...
- The UI never calls the runtime directly — it goes through `packages/sdk`
  (`AgentRuntime`; `DshRuntime` is the primary implementation).
  Pin the dsh version (see `DSH_VERSION` in `packages/sdk/src/types.ts`) and
  bundle it as a sidecar (`runtime/dsh/`, installed by `scripts/dev/fetch-dsh.sh`).
