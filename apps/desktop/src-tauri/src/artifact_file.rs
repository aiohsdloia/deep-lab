...
/// Open an existing folder in an external app: the default terminal
/// (`"terminal"`, the default) or VS Code (`"vscode"`). OS-adapted — macOS
/// opens Terminal.app / VS Code via `open -a` (with the `code` CLI as a
/// fallback), Windows starts `cmd /K` rooted at the folder or the `code` CLI,
/// Linux tries the common desktop terminals / `code`+Flatpak. The UI passes a
/// real session folder (absolute path); anything that is not an existing
/// directory is rejected up front.
#[tauri::command]
pub fn open_folder_in(path: String, target: String) -> Result<(), String> {
    let full = PathBuf::from(&path);
    if !full.is_dir() {
        return Err(format!("not a folder: {}", native_path(&full)));
    }
    // Resolve symlinks so the launched terminal/editor lands on the real path.
    let full = full
        .canonicalize()
        .map_err(|e| format!("cannot resolve folder: {e}"))?;
    match target.as_str() {
        "vscode" => open_in_vscode(&full),
        _ => open_in_terminal(&full),
    }
}

#[cfg(target_os = "macos")]
fn open_in_terminal(full: &Path) -> Result<(), String> {
    // `open -a Terminal <dir>` opens a new Terminal window rooted at <dir>.
...
#[cfg(target_os = "windows")]
fn open_in_terminal(full: &Path) -> Result<(), String> {
    // `cmd /C start "" cmd /K "cd /d <dir>"` — the empty title keeps the first
    // quoted argument from being consumed as the window title, and the outer
    // `start` detaches the new window so spawn returns immediately.
    let dir = native_path(full);
    let script = format!("cd /d \"{dir}\"");
...
fn open_in_terminal(full: &Path) -> Result<(), String> {
    let dir = full.to_string_lossy().into_owned();
    let in_dir = format!("cd \"{dir}\" && exec $SHELL");
    // The common desktop terminals, in order of preference; the first that
    // launches wins. gnome-terminal/konsole/xfce4 take a working-directory
    // flag; the generic emulators take `-e sh -c 'cd … && exec $SHELL'`.
    let attempts: &[(&str, Vec<String>)] = &[
        ("gnome-terminal", vec!["--working-directory".into(), dir.clone()]),
        ("konsole", vec!["--workdir".into(), dir.clone()]),
        ("xfce4-terminal", vec!["--working-directory".into(), dir.clone()]),
        (
            "x-terminal-emulator",
            vec!["-e".into(), "sh".into(), "-c".into(), in_dir.clone()],
        ),
        ("xterm", vec!["-e".into(), "sh".into(), "-c".into(), in_dir]),
    ];
    let mut last_err = "no terminal found".to_string();
...
#[cfg(target_os = "macos")]
fn open_in_vscode(full: &Path) -> Result<(), String> {
    // The `code` CLI needs no PATH when `open -a` is the primary path; try it
    // first for VS Code-family builds (Cursor, etc.), then the registered app.
...
fn open_in_vscode(full: &Path) -> Result<(), String> {
    std::process::Command::new("code")
        ...
        .spawn()
        .map(|_| ())
        .map_err(|e| format!("VS Code CLI (`code`) not found: {e}"))
}

#[cfg(all(unix, not(target_os = "macos")))]
fn open_in_vscode(full: &Path) -> Result<(), String> {
    // `code` CLI first, then the OSS `codium`, then the Flatpak sandbox.
    for bin in ["code", "codium"] {
        if std::process::Command::new(bin).arg(full).spawn().is_ok() {
            return Ok(());
        }
    }
