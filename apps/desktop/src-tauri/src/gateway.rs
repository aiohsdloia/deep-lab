...
fn route(stream: &mut TcpStream, req: &Request, ctx: &Ctx) {
    ...
    if req.method == "GET" && path == "/v1/health" {
        respond_json(stream, 200, "{\"ok\":true,\"service\":\"deeplab-gateway\"}");
        return;
    }
    ...
    if req.method == "GET" {
        ...
        // let the asset resolver's SPA fallback answer dsh API paths
        // (/api/session.list, /api/events.mux, …) with index.html — that would
        // break EventSource (MIME text/html) and every JSON fetch (sessions /
        // runs / notebooks come back as HTML and render blank).
    ...
    // ---- transparent dsh `/api` proxy (the SPA's DshRuntime talks here) ----
    if !authed(req, &ctx.token()) {
        respond_json(stream, 401, "{\"error\":\"unauthorized\"}");
        return;
    }
    ...
    // dsh's own event streams carry the SSE mux/host frames the client folds.
    // They must be proxied as a live stream, not buffered as one response.
    if req.method == "GET" && (path == "/api/events.mux" || path == "/api/events.host") {
