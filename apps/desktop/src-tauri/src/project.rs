...
// `<folder>/.openlab/project.json`. The folder IS the workspace — sessions
...
pub struct ProjectMeta {
    ...
    /// Sidebar accent color for this project (a token like "red"/"blue"), if the
    /// user set one. Absent = default.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
fn meta_file(dir: &Path) -> PathBuf {
    crate::runtime::app_meta_dir(dir).join("project.json")
}
...
fn populate_import(
    ...
    // The source may have carried its own `.openlab/` (if it was ever app-
    ...
    let _ = force_remove_dir_all(&crate::runtime::app_meta_dir(&dir));
    ...
    // provenance/caveats go to an app-owned `.openlab/IMPORTED_FROM.md`; AGENTS.md
    ...
    // `mark_imported` keeps `.openlab/` out of git and snapshots go to the
...
/// stale-environment caveats) go to an app-owned `.openlab/IMPORTED_FROM.md`, so
...
fn record_import_provenance(dir: &Path, source: &Path) {
    ...
    let detail_path = crate::runtime::app_meta_dir(&dir).join("IMPORTED_FROM.md");
    ...
    let line = format!(
        "\n{IMPORT_PROVENANCE_MARKER}\n> Imported into this workspace by copying from \
         `{}` — see `.openlab/IMPORTED_FROM.md` for the original location and caveats.\n",
        source.display()
    );
...
fn set_color_in(base: &Path, id: &str, color: Option<String>) -> Result<(), String> {
    let dir = project_dir_by_id(base, id).ok_or("project not found")?;
    let mut meta = read_meta(&dir).ok_or("not a project folder")?;
    if let Some(c) = color {
        if !matches!(c.as_str(), "red" | "orange" | "yellow" | "green" | "blue" | "purple" | "gray") {
...
#[tauri::command(async)]
pub fn set_project_color(app: AppHandle, id: String, color: Option<String>) -> Result<(), String> {
    set_color_in(&base_workspace_dir(&app)?, &id, color)
}
...
///   remove only the `.openlab/project.json` marker, demoting it to a plain
...
mod tests {
    ...
    fn imported_project_points_at_its_external_source_without_writing_into_it() {
        ...
        assert!(!crate::runtime::app_meta_dir(&ext).join("project.json").exists());
    ...
    #[test]
    fn project_color_roundtrips_and_clears() {
        use super::{set_color_in, set_pinned_in};
        let base = std::env::temp_dir().join(format!("os-project-color-{}", std::process::id()));
        let _ = fs::remove_dir_all(&base);
        fs::create_dir_all(&base).unwrap();

        let (dir, meta) = create_in(&base, "Color Lab").unwrap();
        // Unset by default → absent.
        assert_eq!(read_meta(&dir).unwrap().color, None);
        // Set a token → persisted and readable.
        set_color_in(&base, &meta.id, Some("blue".into())).unwrap();
        assert_eq!(read_meta(&dir).unwrap().color.as_deref(), Some("blue"));
        // Pin survives alongside the color (independent fields).
        set_pinned_in(&base, &meta.id, true).unwrap();
        let meta = read_meta(&dir).unwrap();
        assert_eq!(meta.color.as_deref(), Some("blue"));
        assert_eq!(meta.pinned, Some(true));
        // Clear with `None` → field removed again.
        set_color_in(&base, &meta.id, None).unwrap();
        assert_eq!(read_meta(&dir).unwrap().color, None);
        // Unknown tokens are rejected, not silently stored.
        assert!(set_color_in(&base, &meta.id, Some("neon".into())).is_err());
    ...
    fn provenance_preserves_agents_md_and_is_idempotent() {
        ...
        assert!(agents.contains(".openlab/IMPORTED_FROM.md"));
        assert_eq!(agents.matches(IMPORT_PROVENANCE_MARKER).count(), 1);
        // Full details live in the app-owned file, with the source path.
        let detail = fs::read_to_string(crate::runtime::app_meta_dir(&dir).join("IMPORTED_FROM.md")).unwrap();
    ...
    fn corrupt_meta_reads_as_no_project() {
        ...
        fs::create_dir_all(crate::runtime::app_meta_dir(&dir)).unwrap();
        fs::write(crate::runtime::app_meta_dir(&dir).join("project.json"), "{not json").unwrap();
