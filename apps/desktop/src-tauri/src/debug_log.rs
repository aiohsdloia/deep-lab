...
/// Append one line to debug.log, off the main thread. The log is rotated by
/// reading+rewriting the file once it exceeds a few MB, so a synchronous write
/// can block the UI thread for hundreds of ms during heavy SSE streaming.
#[tauri::command(async)]
pub fn log_debug(app: AppHandle, message: String) {
    tauri::async_runtime::spawn_blocking(move || append(&app, &message));
}
