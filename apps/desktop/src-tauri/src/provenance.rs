...
// version record to <workspace>/.openlab/provenance.jsonl — append-only,
...
pub struct EnvInfo {
    ...
    pub platform: String,
    /// Open Lab app version that recorded this.
    pub app: String,
...
/// `.openlab/env/<hash>.txt`; records carry only the count + hash so the
...
pub struct PackageSnapshot {
    ...
    pub count: u32,
    /// Short content hash; the lockfile is `.openlab/env/<hash>.txt`.
    pub hash: String,
}
...
fn write_lockfile(root: &Path, freeze: &str) -> Result<PackageSnapshot, String> {
    ...
    let dir = crate::runtime::app_meta_dir(&root).join(ENV_DIR);
...
fn store_file(root: &Path) -> PathBuf {
    crate::runtime::app_meta_dir(&root).join(STORE_FILE)
}
...
/// Read a content-addressed package lockfile (`.openlab/env/<hash>.txt`).
...
mod tests {
    ...
    fn lockfile_is_content_addressed_and_deduped() {
        ...
        let lock = crate::runtime::app_meta_dir(&root).join("env").join(format!("{}.txt", s1.hash));
    ...
    fn corrupt_lines_are_skipped_and_content_is_capped() {
        ...
        use std::io::Write;
        let file = crate::runtime::app_meta_dir(&root).join("provenance.jsonl");
