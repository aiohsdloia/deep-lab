...
// <workspace>/.openlab/runs.jsonl: the command, code version (entry scripts
...
const STORE_DIR: &str = crate::runtime::META_DIR;
...
pub struct RunRecord {
    ...
    /// Captured stdout/stderr, content-addressed to `.openlab/logs/<hash>.txt`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
fn is_ignored_dir(name: &str) -> bool {
    matches!(
        name,
        ".git" | STORE_DIR | ".openscience" | "node_modules" | ".venv" | "venv" | "__pycache__" | ".ipynb_checkpoints"
    ) || name.starts_with('.')
}
...
fn runs_file(root: &Path) -> PathBuf {
    crate::runtime::app_meta_dir(&root).join(RUNS_FILE)
}
...
fn write_log(root: &Path, log: &str) -> Option<String> {
    ...
    let dir = crate::runtime::app_meta_dir(&root).join(LOGS_DIR);
...
/// Read a content-addressed run log (`.openlab/logs/<hash>.txt`). `hash` is
...
pub fn read_log(root: &Path, hash: &str) -> Result<String, String> {
    ...
    let path = crate::runtime::app_meta_dir(&root).join(LOGS_DIR).join(format!("{hash}.txt"));
...
pub fn read_runs(root: &Path) -> Vec<RunRecord> {
    let dir = crate::runtime::app_meta_dir(&root);
...
mod tests {
    ...
    fn detects_only_files_modified_in_the_window() {
        ...
        std::fs::create_dir_all(crate::runtime::app_meta_dir(&root)).unwrap();
        std::fs::write(crate::runtime::app_meta_dir(&root).join("ignore.txt"), "x").unwrap();
        ...
        assert!(!paths.iter().any(|p| p.starts_with(crate::runtime::META_DIR) || p.starts_with(".openscience")));
