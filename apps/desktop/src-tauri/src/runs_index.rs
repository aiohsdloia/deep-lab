...
fn db_path(root: &Path) -> PathBuf {
    crate::runtime::app_meta_dir(root).join(DB_FILE)
}
...
/// Every runs log under the base folder: the base's own `.openlab/`, new
...
fn source_files(base: &Path) -> Vec<PathBuf> {
    ...
    dirs.dedup();
    dirs.into_iter()
        .flat_map(|d| [RUNS_FILE, REMOTE_RUNS_FILE].map(|f| crate::runtime::app_meta_dir(&d).join(f)))
        .collect()
}
...
mod tests {
    ...
    fn temp_root(tag: &str) -> PathBuf {
        ...
        std::fs::create_dir_all(crate::runtime::app_meta_dir(&dir)).unwrap();
        dir
    }
    ...
    fn write_log(root: &Path, file: &str, lines: &[&str]) {
        ...
        let mut f = std::fs::OpenOptions::new()
            .create(true)
            .append(true)
            .open(crate::runtime::app_meta_dir(&root).join(file))
            .unwrap();
        for l in lines {
            writeln!(f, "{l}").unwrap();
        }
    ...
    fn merges_remote_runs_and_a_partial_last_line_is_deferred() {
        ...
        use std::io::Write;
        let mut f = std::fs::OpenOptions::new().append(true).open(crate::runtime::app_meta_dir(&root).join(RUNS_FILE)).unwrap();
    ...
    fn indexes_runs_across_session_subfolders_and_filters_by_session() {
        ...
        for (workspace, sess, ts) in [
            ...
            let dir = crate::runtime::app_meta_dir(&root.join(workspace));
