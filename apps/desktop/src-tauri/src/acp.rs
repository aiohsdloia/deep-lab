...
pub fn acp_start(
    ...
    // The bundled DeepSeek Harness launcher reads its model key from
    // DEEPSEEK_API_KEY, and the app already holds that key in its own OpenCode
    // credential store — hand it to the child so the runtime works out of the
    // box with the user's existing provider login. Scoped to the bundled
    // launcher path, never to an arbitrary configured agent command.
    if is_bundled_dsh_launcher(&app, &command, &args) {
        if let Some(key) = deepseek_api_key(&app) {
...
/// Whether this invocation is the bundled DeepSeek Harness launcher — i.e. the
/// only ACP agent whose model key the app may supply from its own credential
/// store. Matched on the resolved resource path, so it is robust to a renamed
/// bundle directory but never matches an arbitrary user-configured command.
fn is_bundled_dsh_launcher(app: &AppHandle, command: &str, args: &[String]) -> bool {
    let launcher = app
        .path()
    ...
    (command == "node" || command.ends_with("/node") || command.ends_with("\\node.exe"))
        && args.iter().any(|a| a == &launcher)
}

/// The DeepSeek API key from the app's own OpenCode credential store, if the
/// user has one configured there. Read-only: the child gets the value in its
/// environment, nothing is written to the workspace or to logs.
fn deepseek_api_key(app: &AppHandle) -> Option<String> {
    let path = crate::runtime::runtime_root(app).ok()?.join("xdg-data").join("opencode").join("auth.json");
    let text = std::fs::read_to_string(path).ok()?;
    deepseek_key_from_auth(&text)
}

/// Parse the OpenCode auth.json shape (`{ "<provider>": { "type", "key" } }`)
/// for the DeepSeek entry's API key.
fn deepseek_key_from_auth(text: &str) -> Option<String> {
...
mod tests {
    ...
    use super::deepseek_key_from_auth;
    ...
    #[test]
    fn deepseek_key_is_read_from_the_app_auth_shape() {
        assert_eq!(
            deepseek_key_from_auth(r#"{"deepseek":{"type":"api","key":"sk-test-123"}}"#),
            Some("sk-test-123".to_string()),
        );
    }

    #[test]
    fn deepseek_key_is_absent_when_not_configured() {
        assert_eq!(deepseek_key_from_auth(r#"{"anthropic":{"type":"api","key":"x"}}"#), None);
        assert_eq!(deepseek_key_from_auth(r#"{"deepseek":{"type":"oauth"}}"#), None);
        assert_eq!(deepseek_key_from_auth("not json"), None);
    }
}
