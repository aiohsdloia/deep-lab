...
// <workspace>/.openlab/compute.json (see compute_machines below).
...
/// `workdir`, when set, is the machine's default remote working directory —
/// the agent places its `jobs/` folder under it (else `openlab` under the SSH
/// home). Never holds shell metacharacters (see `set_compute_workdir`).
/// `os`, when set to `"win"` or `"linux"`, overrides the probe's OS choice;
/// `None` auto-detects.
#[derive(Clone, serde::Serialize, serde::Deserialize)]
pub struct Machine {
    pub host: String,
...
fn legacy_to_machines(hpc_json: &str) -> Vec<Machine> {
    serde_json::from_str::<serde_json::Value>(hpc_json)
        ...
        .map(|host| vec![Machine { host, label: None, caps: None, workdir: None, os: None }])
        .unwrap_or_default()
}
...
fn upsert_machine(list: &mut Vec<Machine>, host: &str, label: Option<String>) {
    if let Some(m) = list.iter_mut().find(|m| m.host == host) {
        ...
        list.push(Machine { host: host.to_string(), label, caps: None, workdir: None, os: None });
    }
}
...
pub(crate) fn compute_path(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    // agent reaches it from its session folder via `../.openlab/compute.json`
    // (the seeded harness points at it), so no per-session copies are kept.
    Ok(crate::runtime::app_meta_dir(&base_workspace_dir(app)?).join(COMPUTE_FILE))
}
...
/// Windows probe (PowerShell over the ssh session's cmd shell). Same token
/// format as PROBE_SCRIPT so `parse_probe` needs no changes; `LOAD` is left out
/// (no /proc/loadavg equivalent without a counter query). One physical line —
/// a newline embedded in the single ssh argument is marshalled inconsistently.
const WIN_PROBE_SCRIPT: &str = "powershell -NoProfile -ExecutionPolicy Bypass -Command \"Write-Output ('OS ' + (Get-CimInstance Win32_OperatingSystem).Caption); Write-Output ('CORES ' + [Environment]::ProcessorCount); $cs=Get-CimInstance Win32_ComputerSystem; Write-Output ('MEMTOTAL ' + $cs.TotalPhysicalMemory); $os=Get-CimInstance Win32_OperatingSystem; Write-Output ('MEMAVAIL ' + ($os.FreePhysicalMemory*1KB)); $d=Get-PSDrive C -ErrorAction SilentlyContinue; if ($d) { Write-Output ('DISKTOTAL ' + ($d.Used+$d.Free)); Write-Output ('DISKFREE ' + $d.Free) }; if (Get-Command nvidia-smi -ErrorAction SilentlyContinue) { nvidia-smi --query-gpu=name,memory.total,memory.used,utilization.gpu --format=csv,noheader,nounits | ForEach-Object { Write-Output ('GPU ' + $_) } }\"";

pub(crate) fn load_machines(app: &AppHandle) -> Result<Vec<Machine>, String> {
    ...
    // `compute_path` above already migrated the metadata folder name, so the
    // legacy file may live under either spelling.
    let base = base_workspace_dir(app)?;
    let legacy = base.join(".openscience").join("hpc.json");
    let migrated = crate::runtime::app_meta_dir(&base).join("hpc.json");
    let legacy = if migrated.exists() { migrated } else { legacy };
...
/// workspace's `.openlab/compute.json` so the agent reads it IN-workspace —
...
pub(crate) fn materialize_active(app: &AppHandle) {
    ...
    write_materialized(
        &crate::runtime::app_meta_dir(&base).join(COMPUTE_FILE),
        &crate::runtime::app_meta_dir(&active),
    );
}
...
pub(crate) fn save_machines(app: &AppHandle, machines: &[Machine]) -> Result<(), String> {
    let path = compute_path(app)?;
...
/// A remote path that is safe to splice into shell commands (the skill builds
/// `mkdir -p <workdir>/jobs/...` and `cd <workdir>`). Path characters only:
/// no quotes, `$`, backticks, `;`, pipes, globs, or a leading `-`.
fn is_safe_workdir(p: &str) -> bool {
    !p.is_empty()
        && !p.starts_with('-')
        && p.chars()
...
/// Set (or clear, with `None`) a machine's default remote working directory.
/// Jobs run under `<workdir>/jobs/<name>`; unset machines default to `openlab`
/// under the SSH home (the remote-compute skill's current behaviour).
#[tauri::command]
pub fn set_compute_workdir(
    app: AppHandle,
    host: String,
    ...
    let machine = machines
        .iter_mut()
        .find(|m| m.host == host)
        .ok_or_else(|| "machine not saved".to_string())?;
    match workdir {
        Some(w) if !is_safe_workdir(&w) => return Err("invalid working directory".into()),
        Some(w) => machine.workdir = Some(w),
        None => machine.workdir = None,
    }
    save_machines(&app, &machines)?;
    materialize_active(&app);
    Ok(())
...
/// A valid machine OS override: `"win"` or `"linux"` (None = auto-detect).
fn is_valid_os(os: &str) -> bool {
    matches!(os, "win" | "linux")
}

/// Set (or clear, with `None`) a machine's OS override. `"win"`/`"linux"` pick
/// the probe script; `None` auto-detects the shell (Windows vs POSIX).
#[tauri::command]
pub fn set_compute_os(app: AppHandle, host: String, os: Option<String>) -> Result<(), String> {
    if let Some(o) = &os {
        if !is_valid_os(o) {
            return Err("invalid os (expected win or linux)".into());
        }
    }
    let mut machines = load_machines(&app)?;
    let machine = machines
...
pub async fn compute_probe(app: AppHandle, host: String) -> Result<ComputeProbe, String> {
    // A saved `os` override picks the probe script; otherwise auto-detect the
    // remote shell (`ver` is a Windows cmd builtin; on POSIX it exits 127).
    let os_override = load_machines(&app)
        .ok()
        .and_then(|ms| ms.iter().find(|m| m.host == host).and_then(|m| m.os.clone()));
    let script = match os_override.as_deref() {
        Some("win") => WIN_PROBE_SCRIPT,
        Some("linux") => PROBE_SCRIPT,
        _ => {
            let (code, stdout, _) = run_ssh(&app, &host, "ver").await?;
            if code == 0 && stdout.contains("Windows") {
                WIN_PROBE_SCRIPT
            } else {
                PROBE_SCRIPT
            }
    ...
    let (code, stdout, stderr) = run_ssh(&app, &host, script).await?;
    if code == 255 {
...
";
    ...
    fn probe_script_is_single_line() {
        ...
        assert!(!super::WIN_PROBE_SCRIPT.contains('\n'), "WIN_PROBE_SCRIPT must be one line");
        assert!(
            super::WIN_PROBE_SCRIPT.contains("nvidia-smi"),
            "windows probe must still query GPUs"
        );
    }

    #[test]
    fn os_validation_accepts_only_win_or_linux() {
        use super::is_valid_os;
        assert!(is_valid_os("win"));
        assert!(is_valid_os("linux"));
        assert!(!is_valid_os("macos"));
        assert!(!is_valid_os(""));
        assert!(!is_valid_os("windows"));
    }

    #[test]
    fn parses_machines_with_os() {
        let json = r#"{"machines":[{"host":"box","workdir":"C:/openlab","os":"win",
            "caps":null}]}"#;
        let m = super::parse_machines(json);
        assert_eq!(m[0].host, "box");
        assert_eq!(m[0].workdir.as_deref(), Some("C:/openlab"));
        assert_eq!(m[0].os.as_deref(), Some("win"));
        // Absent os → None (auto-detect).
        let json = r#"{"machines":[{"host":"box","caps":null}]}"#;
        assert!(super::parse_machines(json)[0].os.is_none());
    }
    ...
    fn parses_machines_with_caps() {
        ...
        assert!(m[0].workdir.is_none(), "absent workdir defaults to none");
    }

    #[test]
    fn parses_machines_with_workdir() {
        let json = r#"{"machines":[{"host":"box","workdir":"/home/user/openlab",
            "caps":null}]}"#;
        let m = super::parse_machines(json);
        assert_eq!(m[0].host, "box");
        assert_eq!(m[0].workdir.as_deref(), Some("/home/user/openlab"));
    }

    #[test]
    fn workdir_validation_rejects_shell_metacharacters() {
        use super::is_safe_workdir;
        assert!(is_safe_workdir("/home/user/openlab"));
        assert!(is_safe_workdir("~/scratch"));
        assert!(is_safe_workdir("C:/openlab"));
        assert!(is_safe_workdir("open lab"));
        assert!(!is_safe_workdir(""));
        assert!(!is_safe_workdir("-x"));
        assert!(!is_safe_workdir("/a;rm -rf /"));
        assert!(!is_safe_workdir("/a$(id)"));
        assert!(!is_safe_workdir("/a`pwd`"));
        assert!(!is_safe_workdir("/a|b"));
        assert!(!is_safe_workdir("/a&b"));
        assert!(!is_safe_workdir("/a b'c"));
        assert!(!is_safe_workdir("/a*b"));
    }
    ...
    #[test]
    fn machine_serialize_roundtrip_keeps_all_fields() {
        // The settings export serializes `Machine` verbatim — every field the
        // app lets the user configure (label, caps, workdir, os) must survive
        // a full JSON round-trip.
        let m = super::Machine {
            host: "box".into(),
            label: Some("GPU box".into()),
            caps: Some(super::Caps {
                cores: Some(16),
                mem_total_bytes: Some(67_516_000_000),
                gpus: vec!["RTX 3090".into()],
                slurm: None,
            }),
            workdir: Some("/home/u/openlab".into()),
            os: Some("linux".into()),
        };
        let json = serde_json::to_string(&m).unwrap();
        let back: super::Machine = serde_json::from_str(&json).unwrap();
        assert_eq!(back.host, "box");
        assert_eq!(back.label.as_deref(), Some("GPU box"));
        assert_eq!(back.workdir.as_deref(), Some("/home/u/openlab"));
        assert_eq!(back.os.as_deref(), Some("linux"));
        assert_eq!(back.caps.as_ref().unwrap().gpus[0], "RTX 3090");
        assert_eq!(back.caps.as_ref().unwrap().cores, Some(16));
    }
