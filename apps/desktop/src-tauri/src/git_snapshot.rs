...
fn snapshot_marker(root: &Path) -> PathBuf {
    // Migrate the legacy git-store name once so pre-rename workspaces keep
    // their snapshot history.
    let new = root.join(".git").join(".deeplab-snapshots");
    let legacy = root.join(".git").join(".openscience-snapshots");
...
/// Written under a workspace's `.openlab/` to opt it out of app-managed
...
fn no_snapshot_marker(root: &Path) -> PathBuf {
    crate::runtime::app_meta_dir(root).join(NO_SNAPSHOT_MARKER)
}
...
/// keep the app's `.openlab/` dir out of the user's `git status` via a local
...
pub fn mark_imported(root: &Path) {
    if root.join(".git").is_dir() {
        // Exclude the current meta dir and the legacy spelling (a workspace may
        // not have been migrated yet when this runs).
        exclude_locally(root, crate::runtime::META_DIR);
        exclude_locally(root, ".deeplab/");
        exclude_locally(root, ".openlab/");
    } else {
        let osdir = crate::runtime::app_meta_dir(root);
...
mod tests {
    ...
    fn importing_a_repo_keeps_it_pristine_and_excludes_the_provenance_dir() {
        ...
        // .openlab/ so our provenance dir never shows in the user's status.
        assert!(!super::no_snapshot_marker(&root).exists());
        let exclude = fs::read_to_string(root.join(".git/info/exclude")).unwrap();
        assert!(exclude.lines().any(|l| l.trim() == crate::runtime::META_DIR));
        ...
        let count = fs::read_to_string(root.join(".git/info/exclude"))
            .unwrap()
            .lines()
            .filter(|l| l.trim() == crate::runtime::META_DIR)
            .count();
        assert_eq!(count, 1);
        let _ = fs::remove_dir_all(&root);
    }
