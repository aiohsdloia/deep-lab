...
import { OpenCodeClient, type OpenCodeEvent } from "@deeplab/sdk";
import { startMockOpenCode, type MockOpenCode } from "@deeplab/sdk/mock-server";
...
describe("image attachments (#88: a vision model must see the figure, not its name)", () => {
  it("sends each attachment as a file part beside the text, and none when there are none", async () => {
    ...
    await client.sendPrompt(sessionId, "inspect this figure", undefined, null, null, undefined, [
