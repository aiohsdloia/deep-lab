 ...
 *  `.deeplab/session.txt`) so skill helpers can attribute remote runs. */
 ...
 *  `.deeplab/project.json`. Legacy root-level projects remain readable. */
export interface ProjectInfo {
  ...
  pinned: boolean;
  /** Sidebar accent color token ("red"/"blue"/…), absent when unset. */
  color?: string;
}
...
/** Which external app a folder should open in. */
export type FolderOpenTarget = "terminal" | "vscode";

/** Open an existing folder in the default terminal (default) or VS Code —
 *  OS-adapted in Rust (Terminal.app / cmd / Linux terminals; `code` + open -a).
 *  Desktop only. */
export async function openFolderIn(
  path: string,
  target: FolderOpenTarget = "terminal",
): Promise<void> {
  if (!isTauri) throw new Error("not running in the desktop app");
  const { invoke } = await import("@tauri-apps/api/core");
  await invoke("open_folder_in", { path, target });
}

/** Detach a screen into its own window (drag a screen tab out). The group was
 *  already handed off via localStorage (`ai4s.detach.<id>`); this window boots
 *  with `?screen=<id>` and claims it. Returns whether the window was opened
 *  (created, or an existing one focused) — the caller only drops the screen
 *  from its own layout when this is true, so a failed detach never loses it.
 *  Desktop only; always false in the browser. */
export async function openScreenWindow(groupId: string, title: string): Promise<boolean> {
  if (!isTauri) return false;
  try {
    const { WebviewWindow } = await import("@tauri-apps/api/webviewWindow");
    const label = `screen-${groupId}`;
    const existing = await WebviewWindow.getByLabel(label);
    if (existing) {
      await existing.setFocus();
      return true;
    }
    // Root path + query, NOT `index.html?...`: the router is a
    // createBrowserRouter whose routes live under `/`. `index.html?screen=x`
    // yields pathname `/index.html`, which matches no route → the NotFound
    // screen renders instead of the app. `/?screen=x` keeps pathname `/` while
    // DETACHED_SCREEN_ID still reads the `screen` query param.
    // No titleBarStyle → a standard native titlebar: the window is draggable
    // by its own title bar, its buttons never overlap the page, and the title
    // (synced to the session) is shown natively instead of an in-app strip.
    await new WebviewWindow(label, {
      url: `/?screen=${encodeURIComponent(groupId)}`,
      title,
      width: 1200,
      height: 820,
      minWidth: 480,
    ...
    void logDebug(`open screen window failed: ${err instanceof Error ? err.message : String(err)}`);
    return false;
  }
}

/** Set the current window's title (desktop only). Detached screen windows use
 *  their native titlebar and keep it synced to the focused session. */
...
/** Export custom providers + remote machines to a user-chosen file (File menu).
 *  Returns the written path, or null when cancelled / not the desktop app. */
export async function exportSettings(): Promise<string | null> {
  if (!isTauri) return null;
  const { invoke } = await import("@tauri-apps/api/core");
  return invoke<string | null>("export_settings");
}

/** Import custom providers + remote machines from a user-chosen file (File menu).
 *  Returns a short summary, or "cancelled" when the dialog was dismissed. */
export async function importSettings(): Promise<string> {
  if (!isTauri) return "cancelled";
  const { invoke } = await import("@tauri-apps/api/core");
  return invoke<string>("import_settings");
}
...
/** Set (or clear, with `null`) a project's sidebar accent color. */
export async function setProjectColor(id: string, color: string | null): Promise<void> {
  if (!isTauri) throw new Error("not running in the desktop app");
  const { invoke } = await import("@tauri-apps/api/core");
  await invoke("set_project_color", { id, color });
}
...
export interface Machine {
  ...
  /** Default remote working directory; jobs land under `<workdir>/jobs/<name>`. */
  workdir: string | null;
  /** Manual OS override: "win" | "linux" | null (auto-detect on probe). */
  os: string | null;
}
...
/** Set (or clear, with null) a machine's default remote working directory. */
export async function setComputeWorkdir(host: string, workdir: string | null): Promise<void> {
  if (!isTauri) throw new Error("not running in the desktop app");
  const { invoke } = await import("@tauri-apps/api/core");
  await invoke("set_compute_workdir", { host, workdir });
}

/** Set (or clear, with null) a machine's OS override: "win" | "linux". */
export async function setComputeOs(host: string, os: string | null): Promise<void> {
  if (!isTauri) throw new Error("not running in the desktop app");
  const { invoke } = await import("@tauri-apps/api/core");
  await invoke("set_compute_os", { host, os });
}

/**
 * Persist the configured multimodal models (image understanding + image
 * generation) next to the sidecar config, so the `image-tools` skill can call
 * them directly even when the session's main model has no vision. No-op in the
 * web client.
 */
export async function setMultimodalModels(
  vision: string | null,
  generate: string | null,
): Promise<void> {
  if (!isTauri) return;
  const { invoke } = await import("@tauri-apps/api/core");
  await invoke("set_multimodal_models", { vision, generate });
}
