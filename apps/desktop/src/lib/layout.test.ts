...
// detachGroup hands the screen to a new window via openScreenWindow (tauri) —
// stub it (window "opens") so the layout logic (handoff + removal) is tested.
vi.mock("@/lib/tauri", () => ({ openScreenWindow: vi.fn(async () => true) }));
import {
  makeLeaf,
  leaves,
  findLeaf,
...
describe("layout store — groups", () => {
  ...
  it("openSessionEphemeral opens the session in the CURRENT screen, no new Screen", () => {
    // g0 shows A; opening P switches the focused pane to P — Screen count stays 1.
    S().openSessionEphemeral("P");
    expect(S().groups).toHaveLength(1);
    expect(S().activeGroupId).toBe("g0");
    expect(leaves(S().tree!).map((l) => l.sessionId)).toEqual(["P"]);
    expect(S().ephemeralGroupId).toBeNull();
  });

  it("openSessionEphemeral focuses an already-shown session instead of duplicating it", () => {
    S().split("row", "B"); // A | B, focus B
    const aId = leaves(S().tree!)[0].id;
    S().focusLeaf(aId);
    S().openSessionEphemeral("A");
    expect(S().groups).toHaveLength(1);
    expect(leaves(S().tree!).map((l) => l.sessionId)).toEqual(["A", "B"]);
    expect(S().focusedLeafId).toBe(aId); // the pane already showing A is focused
  });

  it("openSessionEphemeral fills an empty active Screen instead of stacking another", () => {
    S().addGroup(); // empty, active
    S().openSessionEphemeral("B");
    expect(S().groups).toHaveLength(2); // no third Screen
    expect(leaves(S().tree!).map((l) => l.sessionId)).toEqual(["B"]);
  });
  ...
  it("detachGroup hands the screen off and drops it from this window", async () => {
    S().addGroup(); // second (empty) group, active
    const gid = S().groups[0].id; // g0 still holds "A"
    await S().detachGroup(gid);
    // The screen moved into its own window — this window keeps the neighbor.
    expect(S().groups).toHaveLength(1);
    expect(S().groups[0].id).not.toBe(gid);
    expect(S().activeGroupId).toBe(S().groups[0].id);
    // The handed-off group is staged for the new window's boot.
    const raw = window.localStorage.getItem(`ai4s.detach.${gid}`);
    expect(raw).toBeTruthy();
    expect(JSON.parse(raw!).id).toBe(gid);
  });

  it("a failed detach keeps the screen in this window (nothing is lost)", async () => {
    // The window fails to open → the screen must NOT be dropped.
    const openScreenWindow = (await import("@/lib/tauri")).openScreenWindow as ReturnType<typeof vi.fn>;
    openScreenWindow.mockResolvedValueOnce(false);
    S().addGroup(); // second group so g0 is not the sole group
    const gid = S().groups[0].id;
    await S().detachGroup(gid);
    expect(S().groups).toHaveLength(2); // g0 still here
    expect(S().groups[0].id).toBe(gid);
  });

  it("detaching the sole screen empties it (a window always has a group)", async () => {
    const gid = S().activeGroupId;
    await S().detachGroup(gid);
    expect(S().groups).toHaveLength(1);
    expect(S().groups[0].id).toBe(gid);
    expect(S().tree).toBeNull(); // emptied → onboarding
  });
