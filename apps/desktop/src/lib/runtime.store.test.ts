...
const mocks = vi.hoisted(() => ({
  ...
  /** Constructor options every DshRuntime was created with. */
...
vi.mock("@deeplab/sdk", async (importOriginal) => {
  ...
  class DshRuntime {
    ...
    async moveSession(id: string, directory: string) {
      ...
      if (mocks.failMove) throw new Error("MoveSessionError: Destination directory belongs to another project");
    }
    async listSkills() {
      return [{ name: "stub" }];
    }
...
import { DRAFT_KEY, loadQueues, rootSessionOf, useRuntimeStore } from "./runtime";
...
beforeEach(async () => {
  ...
  window.localStorage.removeItem("ai4s.sessions.interrupted.v1");
...
describe("agent artifact presentation targets", () => {
  it("surfaces a presented panel file as a locatable card in the conversation thread", async () => {
    useRuntimeStore.setState({
      currentId: "ses_cur",
      sessions: [{ id: "ses_cur", title: "Cur", directory: "/ws/cur" }],
    });
    mocks.fireEvent({
      type: "tool.updated",
      sessionId: "ses_cur",
      callId: "present-panel-cur",
      tool: "present_artifact",
      status: "success",
      input: {
        path: "test_revised.docx",
        display: "panel",
        placement: "right",
    ...
    const blocks = useRuntimeStore.getState().threads["ses_cur"]?.blocks ?? [];
    const card = blocks.find((b) => b.kind === "artifact" && b.path === "test_revised.docx");
    expect(card).toBeTruthy();
    expect((card as { artifact: string }).artifact).toBe("report");
  });
...
describe("runtime authentication", () => {
  ...
  it("connect() points the SDK client at the sidecar with the workspace scope", async () => {
    // dsh serves loopback-only with a browser-trust fence — no sidecar
    // password exists. The client carries the workspace directory instead.
...
describe("stale running locks and interrupt", () => {
  ...
  it("records an interrupted session for the sidebar dot and clears it on a fresh turn", async () => {
    await useRuntimeStore.getState().sendPrompt("hi");
    await useRuntimeStore.getState().interrupt();
    expect(useRuntimeStore.getState().interruptedSessions["ses_new"]).toBe(true);
    // Persisted so the yellow dot survives a restart.
    expect(JSON.parse(window.localStorage.getItem("ai4s.sessions.interrupted.v1") ?? "{}")).toEqual({
      ses_new: true,
    });
    // A fresh turn clears the dot.
    await useRuntimeStore.getState().sendPrompt("again");
    expect(useRuntimeStore.getState().interruptedSessions["ses_new"]).toBeUndefined();
    expect(JSON.parse(window.localStorage.getItem("ai4s.sessions.interrupted.v1") ?? "{}")).toEqual({});
  });

  it("re-derives the interrupted dot from a frozen step of a FINISHED turn when reopened", async () => {
    // Turn over (completed) but a tool never finished → genuinely interrupted.
    mocks.messages = [
      { role: "user", parts: [{ type: "text", text: "hi" }] },
      {
        role: "assistant",
        parts: [{ type: "tool", state: { status: "running", title: "bash" } }],
        completed: Date.now(),
      },
    ];
    await useRuntimeStore.getState().openSession("ses_frozen");
    expect(useRuntimeStore.getState().interruptedSessions["ses_frozen"]).toBe(true);
  });

  it("does NOT mark a still-streaming turn (live on another client) as interrupted", async () => {
    // The desktop app is mid-tool; reopening the session on the web must not
    // flag it interrupted — it is running, not dead.
    mocks.messages = [
      { role: "user", parts: [{ type: "text", text: "hi" }] },
      { role: "assistant", parts: [{ type: "tool", state: { status: "running", title: "bash" } }] },
    ];
    await useRuntimeStore.getState().openSession("ses_live_frozen");
    expect(useRuntimeStore.getState().interruptedSessions["ses_live_frozen"]).toBeUndefined();
  });

  it("clears a stale interrupted dot when the reopened history finished", async () => {
    useRuntimeStore.setState({ interruptedSessions: { ses_done: true } });
    mocks.messages = [
      { role: "user", parts: [{ type: "text", text: "hi" }] },
      { role: "assistant", parts: [{ type: "text", text: "ok" }], completed: Date.now() },
    ];
    await useRuntimeStore.getState().openSession("ses_done");
    expect(useRuntimeStore.getState().interruptedSessions["ses_done"]).toBeUndefined();
  });
  ...
  it("drops a history-seeded lock to stopped when no live event arrives (app-restart ghost)", async () => {
    vi.useFakeTimers();
    try {
      // A turn orphaned by an app restart: the sidecar was SIGKILLed mid-answer,
      // so its last message is incomplete but nothing will ever stream again.
      mocks.messages = [
        { role: "user", parts: [{ type: "text", text: "hi" }] },
        { role: "assistant", parts: [{ type: "text", text: "thinking…" }] }, // no `completed`
      ];
      const p = useRuntimeStore.getState().openSession("ses_ghost");
      await vi.advanceTimersByTimeAsync(0);
      await p;
      expect(useRuntimeStore.getState().runningSessions["ses_ghost"]).toBe(true);
      // No stream event arrives → the run-proof grace elapses → stopped.
      await vi.advanceTimersByTimeAsync(4000);
      expect(useRuntimeStore.getState().runningSessions["ses_ghost"]).toBeUndefined();
    } finally {
      vi.useRealTimers();
    }
  });

  it("keeps a history-seeded lock running when a live stream event arrives", async () => {
    vi.useFakeTimers();
    try {
      // The sidecar survived (webview reload): the turn really is streaming.
      mocks.messages = [
        { role: "user", parts: [{ type: "text", text: "hi" }] },
        { role: "assistant", parts: [{ type: "text", text: "thinking…" }] },
      ];
      const p = useRuntimeStore.getState().openSession("ses_live");
      await vi.advanceTimersByTimeAsync(0);
      await p;
      expect(useRuntimeStore.getState().runningSessions["ses_live"]).toBe(true);
      // A live event proves the turn — the grace no longer applies.
      mocks.fireEvent({ type: "text.updated", sessionId: "ses_live", text: "more" });
      await vi.advanceTimersByTimeAsync(4000);
      expect(useRuntimeStore.getState().runningSessions["ses_live"]).toBe(true);
    } finally {
      vi.useRealTimers();
...
// The per-session prompt queue: queued prompts run one after another, each
// starting when the previous turn completes (session.idle), FIFO, execute-one-
// remove-one. Enqueueing while idle starts the first item immediately.
// The per-session prompt queue: queued instructions run one after another, each
// starting when the previous turn completes (session.idle), FIFO, execute-one-
// remove-one. Every item carries a run plan (delay before it starts) and a
// repeat count (default 1, max 99).
describe("per-session prompt queue", () => {
  it("queues a prompt while running and auto-starts it on session.idle", async () => {
    await useRuntimeStore.getState().sendPrompt("hi");
    expect(useRuntimeStore.getState().runningSessions["ses_new"]).toBe(true);

    // Queue while the turn is running — it must NOT start yet.
    useRuntimeStore.getState().enqueuePrompt("ses_new", "then do B");
    expect(useRuntimeStore.getState().queues["ses_new"]).toEqual([
      { text: "then do B", delayMin: 0, repeats: 1, readyAt: expect.any(Number) },
    ]);
    expect(mocks.sendPromptSpy).toHaveBeenCalledTimes(1); // only "hi" so far
    ...
    expect(users.map((u) => (u as { text: string }).text)).toEqual(["hi", "then do B"]);
  });
  ...
  it("drains the queue after a SYNC shell turn settles (idle beat the POST)", async () => {
    // Establish the session, then end that turn cleanly.
    await useRuntimeStore.getState().sendPrompt("hi");
    mocks.fireEvent({ type: "session.idle", sessionId: "ses_new" });
    mocks.sendPromptSpy.mockClear(); // ignore the setup send

    // A "!" shell turn: the mock fires session.idle synchronously inside the
    // POST while the send lock is still held, so the idle handler's drainQueue
    // must wait for the lock to clear — the performTurn finally is that second
    // chance. Regression for "queue never auto-submits after a shell/command".
    const p = useRuntimeStore.getState().runShell("pwd"); // don't await yet
    // Queue a follow-up while the shell turn is still settling.
    useRuntimeStore.getState().enqueuePrompt("ses_new", "after shell");
    expect(useRuntimeStore.getState().queues["ses_new"]!.map((x) => x.text)).toEqual([
      "after shell",
    ]);
  ...
  it("does not drain while a question is pending", async () => {
    // A session blocked on a question is not free to start a queued turn.
    mocks.fireEvent({
      type: "question.asked",
      sessionId: "ses_new",
      requestId: "que_9",
    ...
    useRuntimeStore.getState().enqueuePrompt("ses_new", "queued");
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(mocks.sendPromptSpy).not.toHaveBeenCalled();
    expect(useRuntimeStore.getState().queues["ses_new"]![0].text).toBe("queued");
  });

  it("runs an item `repeats` times before dropping it", async () => {
    useRuntimeStore.getState().enqueuePrompt("ses_new", "repeat me", { repeats: 3 });
    ...
    mocks.fireEvent({ type: "session.idle", sessionId: "ses_new" });
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(mocks.sendPromptSpy).toHaveBeenCalledTimes(2);
    expect(useRuntimeStore.getState().queues["ses_new"]![0].repeats).toBe(1);

    mocks.fireEvent({ type: "session.idle", sessionId: "ses_new" });
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(mocks.sendPromptSpy).toHaveBeenCalledTimes(3);
    expect(useRuntimeStore.getState().queues["ses_new"] ?? []).toEqual([]);
  });

  it("holds a delayed item until its run plan time, then sends it", async () => {
    vi.useFakeTimers();
    try {
      vi.setSystemTime(new Date("2026-01-01T00:00:00Z"));
      useRuntimeStore.getState().enqueuePrompt("ses_new", "later", { delayMin: 5 });
      // Not ready yet — nothing sent, item stays queued with the planned start.
      expect(mocks.sendPromptSpy).not.toHaveBeenCalled();
      expect(useRuntimeStore.getState().queues["ses_new"]).toHaveLength(1);
      expect(useRuntimeStore.getState().queues["ses_new"]![0].readyAt).toBe(
  ...
  it("clamps repeat counts to [1, 99] and delays to the allowed set", async () => {
    // Keep the session running so enqueues accumulate instead of auto-draining.
    await useRuntimeStore.getState().sendPrompt("hi");
    const st = useRuntimeStore.getState();
    st.enqueuePrompt("ses_new", "x", { repeats: 0 });
    expect(useRuntimeStore.getState().queues["ses_new"]![useRuntimeStore.getState().queues["ses_new"]!.length - 1].repeats).toBe(1);
    st.enqueuePrompt("ses_new", "x", { repeats: 500 });
    expect(useRuntimeStore.getState().queues["ses_new"]![useRuntimeStore.getState().queues["ses_new"]!.length - 1].repeats).toBe(99);
    st.enqueuePrompt("ses_new", "x", { delayMin: 7 });
    expect(useRuntimeStore.getState().queues["ses_new"]![useRuntimeStore.getState().queues["ses_new"]!.length - 1].delayMin).toBe(0);
    st.enqueuePrompt("ses_new", "x", { delayMin: 30 });
    expect(useRuntimeStore.getState().queues["ses_new"]![useRuntimeStore.getState().queues["ses_new"]!.length - 1].delayMin).toBe(30);
  });

  it("edit actions update the queue (remove, update, patch plan, append, set, clear)", () => {
    const item = (text: string): QueueItem => ({ text, delayMin: 0, repeats: 1 });
    useRuntimeStore.setState({ queues: { ses_new: [item("A"), item("B"), item("C")] } });
    const st = useRuntimeStore.getState();
    st.removeQueuedPrompt("ses_new", 1);
    expect(useRuntimeStore.getState().queues["ses_new"]!.map((x) => x.text)).toEqual(["A", "C"]);
    st.updateQueuedPrompt("ses_new", 0, "A2");
    expect(useRuntimeStore.getState().queues["ses_new"]!.map((x) => x.text)).toEqual(["A2", "C"]);
    st.updateQueueItem("ses_new", 0, { delayMin: 10 });
    expect(useRuntimeStore.getState().queues["ses_new"]![0].delayMin).toBe(10);
    st.updateQueueItem("ses_new", 0, { repeats: 99 });
    expect(useRuntimeStore.getState().queues["ses_new"]![0].repeats).toBe(99);
    st.appendQueuedPrompt("ses_new", "D");
    expect(useRuntimeStore.getState().queues["ses_new"]!.map((x) => x.text)).toEqual(["A2", "C", "D"]);
    st.setQueue("ses_new", ["X", " ", "Y"]);
    expect(useRuntimeStore.getState().queues["ses_new"]!.map((x) => x.text)).toEqual(["X", "Y"]);
    st.clearQueue("ses_new");
    expect(useRuntimeStore.getState().queues["ses_new"]).toBeUndefined();
  });

  it("migrates legacy plain-string queues on load", () => {
    window.localStorage.setItem(
      "ai4s.queues.v1",
      JSON.stringify({ ses_legacy: ["old prompt"] }),
    );
    expect(loadQueues()).toEqual({
      ses_legacy: [{ text: "old prompt", delayMin: 0, repeats: 1 }],
    });
    window.localStorage.removeItem("ai4s.queues.v1");
  });

  it("auto-saves the queue to localStorage and recovers it after a crash", () => {
    // appendQueuedPrompt does not auto-drain, so the entry stays queued.
    useRuntimeStore.getState().appendQueuedPrompt("ses_persist", "keep me");
    expect(useRuntimeStore.getState().queues["ses_persist"]).toEqual([
      { text: "keep me", delayMin: 0, repeats: 1, readyAt: expect.any(Number) },
    ]);
    const saved = JSON.parse(window.localStorage.getItem("ai4s.queues.v1") ?? "{}");
    expect(saved.ses_persist[0].text).toBe("keep me");

    // A "crash": memory wiped, then the store re-initializes from storage.
    useRuntimeStore.setState({ queues: {} });
    useRuntimeStore.setState({ queues: loadQueues() });
    expect(useRuntimeStore.getState().queues["ses_persist"]?.[0].text).toBe("keep me");

    // Clearing the queue also clears the persisted copy.
    useRuntimeStore.getState().clearQueue("ses_persist");
    expect(JSON.parse(window.localStorage.getItem("ai4s.queues.v1") ?? "{}")).toEqual({});
  });

  it("drop of an executed item is persisted; deleting the session drops its queue", () => {
    const item = (text: string): QueueItem => ({ text, delayMin: 0, repeats: 1 });
    useRuntimeStore.setState({ queues: { ses_persist: [item("one"), item("two")] } });
    useRuntimeStore.getState().removeQueuedPrompt("ses_persist", 0);
    const saved = JSON.parse(window.localStorage.getItem("ai4s.queues.v1") ?? "{}");
    expect(saved.ses_persist.map((x: QueueItem) => x.text)).toEqual(["two"]);
    useRuntimeStore.setState({ queues: { ses_persist: [item("two")] } });
    useRuntimeStore.getState().deleteSession("ses_persist");
    expect(JSON.parse(window.localStorage.getItem("ai4s.queues.v1") ?? "{}")).toEqual({});
  });
});

describe("idle replay re-lock guard", () => {
  it("a replayed activity right after session.idle does not re-lock the finished turn", async () => {
    const id = await useRuntimeStore.getState().sendPrompt("hi");
    expect(useRuntimeStore.getState().runningSessions[id!]).toBe(true);
    // The turn finishes: idle clears the lock and arms the idle guard.
    mocks.fireEvent({ type: "session.idle", sessionId: id! });
    expect(useRuntimeStore.getState().runningSessions[id!]).toBeUndefined();
    // A sidecar restart replays the turn's tail (a message.agent after idle) —
    // the "sign of life" re-lock must skip the just-idled session.
    mocks.fireEvent({ type: "message.agent", sessionId: id! });
    expect(useRuntimeStore.getState().runningSessions[id!]).toBeUndefined();
    // A fresh send clears the guard, so the next turn's activity re-locks.
    await useRuntimeStore.getState().sendPrompt("again");
    expect(useRuntimeStore.getState().runningSessions[id!]).toBe(true);
  });

  it("still re-locks a session whose lock was lost on reload (no idle seen here)", async () => {
    const id = await useRuntimeStore.getState().sendPrompt("hi");
    useRuntimeStore.setState({ runningSessions: {} }); // reload: locks gone
    mocks.fireEvent({
      type: "tool.updated",
      sessionId: id!,
      callId: "c1",
...
describe("image-understanding routing", () => {
  const lastModel = () => {
    const calls = mocks.sendPromptFullSpy.mock.calls;
    return calls[calls.length - 1]?.[3] as string | null | undefined;
  };

  it("routes a turn that references an image to the chosen vision model", async () => {
    useRuntimeStore.getState().setVisionModel("zhipu/glm-4v-flash");
    useRuntimeStore.setState({ defaultModel: "anthropic/claude-sonnet-5" });
    await useRuntimeStore.getState().sendPrompt("explain the figure in figures/heatmap.png");
    expect(lastModel()).toBe("zhipu/glm-4v-flash");
  });

  it("falls back to the free Zhipu vision model when none is chosen and zhipu is configured", async () => {
    mocks.providers = [
      { id: "zhipu", name: "Zhipu", models: [{ id: "glm-4v-flash", name: "GLM-4V Flash" }] },
    ];
    useRuntimeStore.setState({
      defaultModel: "anthropic/claude-sonnet-5",
      providers: mocks.providers as never,
    });
    await useRuntimeStore.getState().sendPrompt("see notes/scan.png");
    expect(lastModel()).toBe("zhipu/glm-4v-flash");
  });

  it("keeps the session model for a plain text turn even with a vision model set", async () => {
    useRuntimeStore.getState().setVisionModel("zhipu/glm-4v-flash");
    useRuntimeStore.setState({ defaultModel: "anthropic/claude-sonnet-5" });
    await useRuntimeStore.getState().sendPrompt("hello");
    expect(lastModel()).toBe("anthropic/claude-sonnet-5");
  });

  it("does NOT route generation requests to a chat image model (skill handles it)", async () => {
    useRuntimeStore.getState().setVisionModel("zhipu/glm-4v-flash");
    useRuntimeStore.setState({ defaultModel: "anthropic/claude-sonnet-5" });
    await useRuntimeStore.getState().sendPrompt("画一张关于基因表达的热力图");
    // Still the session model — no cogview chat routing, no SSE failure.
    expect(lastModel()).toBe("anthropic/claude-sonnet-5");
  });
});

describe("moving a session into a project", () => {
  it("keeps the optimistic project folder even if the next refresh is stale", async () => {
    useRuntimeStore.setState({
      sessions: [{ id: "ses_move", title: "A", directory: "/base/old" }],
    });
    // The sidecar hasn't committed the move yet — a refresh right now returns
    // the OLD folder. It must NOT yank the session out of its project row.
    mocks.sessionList = [{ id: "ses_move", title: "A", directory: "/base/old" }];

    const ok = await useRuntimeStore.getState().moveSessionToWorkspace("ses_move", "/base/proj-a");
    expect(ok).toBe(true);
    // Optimistic: the sidebar groups it under the project immediately.
    expect(useRuntimeStore.getState().sessions.find((s) => s.id === "ses_move")?.directory).toBe(
      "/base/proj-a",
    );
    // A stale refresh resolves AFTER the optimistic set — the pending overlay
    // holds the new folder instead of reverting ("project sometimes doesn't update").
    await useRuntimeStore.getState().refreshSessions();
    expect(useRuntimeStore.getState().sessions.find((s) => s.id === "ses_move")?.directory).toBe(
      "/base/proj-a",
    );

    // The server commits; the association holds (it is app-authoritative).
    mocks.sessionList = [{ id: "ses_move", title: "A", directory: "/base/proj-a" }];
    await useRuntimeStore.getState().refreshSessions();
    expect(useRuntimeStore.getState().sessions.find((s) => s.id === "ses_move")?.directory).toBe(
      "/base/proj-a",
    );
  ...
  it("keeps the association even when the sidecar refuses the cross-project move", async () => {
    useRuntimeStore.setState({
    ...
    const ok = await useRuntimeStore.getState().moveSessionToWorkspace("ses_x", "/base/proj-x");
    expect(ok).toBe(false); // server truth unchanged
    expect(useRuntimeStore.getState().sessions.find((s) => s.id === "ses_x")?.directory).toBe(
      "/base/proj-x",
    );
    // A refresh re-reads the server's old directory — the persistent override
    // keeps the project grouping instead of reverting.
    await useRuntimeStore.getState().refreshSessions();
    expect(useRuntimeStore.getState().sessions.find((s) => s.id === "ses_x")?.directory).toBe(
      "/base/proj-x",
    );
    // The association is persisted, so it survives an app restart.
    expect(window.localStorage.getItem("ai4s.session.dirs.v1")).toContain("/base/proj-x");
  });

  it("persists the association across a simulated restart", async () => {
    useRuntimeStore.setState({
      sessions: [{ id: "ses_p", title: "C", directory: "/base/old" }],
    });
    await useRuntimeStore.getState().moveSessionToWorkspace("ses_p", "/base/proj-p");
    // A "restart": the in-memory overrides map reloads from localStorage.
    useRuntimeStore.setState({ sessions: [] });
    mocks.sessionList = [{ id: "ses_p", title: "C", directory: "/base/old" }];
    await useRuntimeStore.getState().refreshSessions();
    expect(useRuntimeStore.getState().sessions.find((s) => s.id === "ses_p")?.directory).toBe(
      "/base/proj-p",
    );
  });

  it("deleting the session clears its persisted association", async () => {
    await useRuntimeStore.getState().moveSessionToWorkspace("ses_del", "/base/proj-d");
    await useRuntimeStore.getState().deleteSession("ses_del");
    const saved = JSON.parse(window.localStorage.getItem("ai4s.session.dirs.v1") ?? "{}");
    expect(saved).not.toHaveProperty("ses_del");
  });
});
