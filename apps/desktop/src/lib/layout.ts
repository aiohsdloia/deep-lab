...
import { openScreenWindow } from "./tauri";
...
//
// A screen can be DETACHED into its own window (drag its tab out). That window
// boots with `?screen=<id>` and keeps its single-screen layout under a separate
// key, so it never clobbers the main window's layout. The group itself is
// handed off via `ai4s.detach.<id>` — localStorage is shared across the app's
// windows (same origin), so the new window claims it on boot.
export const DETACHED_SCREEN_ID: string | null = (() => {
  if (typeof window === "undefined") return null;
  return new URLSearchParams(window.location.search).get("screen");
})();
/** True in a screen window that was detached from the main window. */
export const isDetachedScreen = !!DETACHED_SCREEN_ID;

const LAYOUT_KEY = DETACHED_SCREEN_ID
  ? `ai4s.layout.detached.${DETACHED_SCREEN_ID}`
  : "ai4s.layout.v2";
const detachKey = (id: string) => `ai4s.detach.${id}`;

/** Claim the group a main window handed off for a detached screen window. */
function loadDetached(): LayoutGroup | null {
  if (!DETACHED_SCREEN_ID || typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(detachKey(DETACHED_SCREEN_ID));
    window.localStorage.removeItem(detachKey(DETACHED_SCREEN_ID));
    if (!raw) return null;
    const g = JSON.parse(raw) as LayoutGroup;
    if (typeof g.id !== "string" || typeof g.name !== "string") return null;
    if (g.tree !== null && !isNode(g.tree)) return null;
    g.zoomedLeafId = null; // zoom is transient
    return g;
  } catch {
    return null;
  }
...
interface LayoutState {
  ...
  zoomedLeafId: string | null;
  /** The "tentative" (preview) screen. openSessionEphemeral reuses it while it
   *  is active; any real interaction — typing, sending, splitting, docking, or
   *  switching to another screen — PINS it (clears this) so it becomes a normal
   *  screen. No sidebar click creates a fresh one anymore: a session click
   *  opens in the CURRENT screen (see openSessionEphemeral). Not persisted (a
   *  relaunch restores every screen as pinned). Invariant: non-null ⟹ ===
   *  activeGroupId. */
  ephemeralGroupId: string | null;
  /** Open `sessionId` in the current screen: focus its pane if it is already
   *  shown there, fill an empty screen, else replace the focused pane's
   *  conversation with this session. If a tentative preview screen is active it
   *  is reused (its single pane swaps instead). The sidebar-click entry — a
   *  session click SWITCHES the view to that session, it never spawns a new
   *  screen (#3). */
  ...
  /** Detach a screen into its OWN window (drag its tab out): hand the group off
   *  via localStorage, open a new window for it, then drop it from this one.
   *  The screen is only dropped once the window is confirmed opened, so a
   *  failed detach never loses it. The sole group is emptied (≥1 always). */
  detachGroup: (groupId: string, title?: string) => Promise<void>;
...
export const useLayoutStore = create<LayoutState>((set, get) => {
  // Restore the saved workspace, else start as a single pane seeded from the
  // URL. A detached screen window starts from the group the main window handed
  // off (loadDetached claims it and removes the handoff key).
  const detached = loadDetached();
  const restored = detached ? null : loadPersisted();
  const root = restored ? null : makeLeaf(initialSessionId());
  const initGroups: LayoutGroup[] = detached
    ? [detached]
    : restored
      ? restored.groups
      : [{ id: genGroupId(), name: "", tree: root, focusedLeafId: root!.id, zoomedLeafId: null }];
  const initActiveId = detached ? detached.id : restored ? restored.activeGroupId : initGroups[0].id;
  ...
  return {
    ...
    openSessionEphemeral: (sessionId) =>
      set((s) => {
        ...
        // No tentative screen — open the session in the CURRENT screen rather
        // than stacking a new one: a sidebar session click SWITCHES the view to
        // that session, it doesn't create a new screen.
        if (s.tree) {
          // Already shown in this screen? Just focus that pane.
          const existing = leaves(s.tree).find((l) => l.sessionId === sessionId);
          if (existing) {
            const groups = s.groups.map((g) =>
              g.id === s.activeGroupId
                ? { ...g, focusedLeafId: existing.id, zoomedLeafId: null }
                : g,
            );
            persist(groups, s.activeGroupId);
            return {
              ...
              zoomedLeafId: null,
              ephemeralGroupId: s.ephemeralGroupId,
            };
          }
          // Otherwise replace the focused pane's conversation with this session
          // (switching sessions in place — never clobbering a running pane's
          // layout, just its bound session).
          const target =
            s.focusedLeafId && findLeaf(s.tree, s.focusedLeafId)
              ? s.focusedLeafId
              : leaves(s.tree)[0].id;
          const tree = setLeafSession(s.tree, target, sessionId);
          const groups = s.groups.map((g) =>
            g.id === s.activeGroupId
              ? { ...g, tree, focusedLeafId: target, zoomedLeafId: null }
              : g,
          );
          persist(groups, s.activeGroupId);
          return {
        ...
        // Empty active screen: fill it (no second empty screen beside it).
        const groups = s.groups.map((g) =>
          g.id === s.activeGroupId
            ? { ...g, tree: leaf, focusedLeafId: leaf.id, zoomedLeafId: null }
            : g,
        );
        persist(groups, s.activeGroupId);
        return {
    ...
    detachGroup: async (groupId, title) => {
      const g = get().groups.find((x) => x.id === groupId);
      if (!g) return;
      // Hand the group off to the new window BEFORE creating it: localStorage is
      // synchronous, and the new window claims it on boot. The group's own
      // window boots with ?screen=<id> and reads this key (shared origin).
      try {
        window.localStorage.setItem(detachKey(groupId), JSON.stringify(g));
      } catch {
        /* storage unavailable — the new window just boots empty */
      }
      // Only drop the screen once the window is actually open — a failed
      // detach keeps it right here instead of losing it.
      const opened = await openScreenWindow(groupId, title ?? (g.name || "Screen"));
      if (!opened) return;
      set((s) => {
        const ephemeralGroupId = s.ephemeralGroupId === groupId ? null : s.ephemeralGroupId;
        // Never zero groups — the sole group is emptied instead of removed, so
        // the main window falls back to onboarding after detaching its last screen.
        if (s.groups.length <= 1) {
          const only: LayoutGroup = { ...s.groups[0], tree: null, focusedLeafId: null, zoomedLeafId: null };
          return { groups: [only], activeGroupId: only.id, tree: null, focusedLeafId: null, zoomedLeafId: null, ephemeralGroupId };
        }
        const idx = s.groups.findIndex((x) => x.id === groupId);
        if (idx < 0) return {};
        const groups = s.groups.filter((x) => x.id !== groupId);
        const active =
          s.activeGroupId === groupId
            ? groups[Math.min(idx, groups.length - 1)]
            : groups.find((x) => x.id === s.activeGroupId)!;
        return {
          groups,
          activeGroupId: active.id,
          tree: active.tree,
