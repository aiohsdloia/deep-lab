...
// records in `.deeplab/runs.jsonl`. Unlike file provenance (which records
...
import type { RunArtifact, RunRecord } from "@deeplab/shared";
import type { ToolUpdatedEvent } from "@deeplab/sdk";
...
export function runInputFromEvent(event: ToolUpdatedEvent): RunInput | null {
  ...
  // record them instead (into .deeplab/remote-runs.jsonl) with real remote
...
export function reproduceRunPrompt(r: RunRecord): string {
  ...
  if (env) {
    ...
    if (env.packages)
      parts.push(
        `The environment had ${env.packages.count} installed Python packages, pinned in \`.deeplab/env/${env.packages.hash}.txt\` — install matching versions from that lockfile if the result differs.`,
      );
  }
