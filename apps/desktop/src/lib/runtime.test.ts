...
describe("historyToThread", () => {
  ...
  it("never spins in history: frozen steps of a FINISHED turn render quiet + one interrupted line", () => {
    // The turn is over (completed) yet a tool never finished — a genuinely stuck
    // turn. It renders the frozen steps quietly and says so once. A turn that is
    // still STREAMING is live (or a ghost) and must NOT be labeled interrupted.
    const msgs: HistoryMessage[] = [
  ...
  it("does NOT label the session interrupted when only an EARLIER turn froze", () => {
    // A stale running step from a previous aborted/closed turn (e.g. a long bash
    // command left running when the app quit) must not mark a session whose LAST
    // turn actually completed — otherwise every reopen shows a false
    // "Interrupted — this turn did not finish".
    const msgs: HistoryMessage[] = [
      { role: "user", parts: [{ type: "text", text: "first" }] },
      {
        role: "assistant",
        parts: [{ type: "tool", tool: "bash", state: { status: "running", title: "sleep 590" } }],
      },
      { role: "user", parts: [{ type: "text", text: "second" }] },
      {
        role: "assistant",
        parts: [
          { type: "tool", tool: "bash", state: { status: "completed", title: "ls -la" } },
          { type: "text", text: "done." },
        ],
      },
    ];
    const t = historyToThread(msgs);
    // The earlier frozen step renders quietly, but no "Interrupted" status line.
    expect(t.blocks[t.blocks.length - 1]).toMatchObject({ kind: "agent" });
    expect(
      t.blocks.some((b) => b.kind === "status-line" && b.tone === "error"),
    ).toBe(false);
  });
  ...
  it("does not label a STILL-STREAMING turn as Interrupted (a live run in history)", () => {
    // A turn mid-flight (e.g. the desktop app is running a long tool right now)
    // has an incomplete last message with a "running" tool step. Reloading that
    // session must NOT say "Interrupted" — the turn is live, not dead.
    const msgs: HistoryMessage[] = [
      { role: "user", parts: [{ type: "text", text: "hi" }] },
      {
        role: "assistant",
        parts: [{ type: "tool", tool: "bash", state: { status: "running", title: "sleep 10" } }],
        // no `completed` → still streaming
      },
    ];
    const t = historyToThread(msgs);
    expect(t.blocks.some((b) => b.kind === "status-line" && b.tone === "error")).toBe(false);
  });

  it("surfaces a panel-presented file as a locatable artifact card on reload", () => {
    // Mirrors the real "修订test.doc" history: the agent packs test_revised.docx
    // via a script and presents it as a panel — on reload it must come back as
    // a normal artifact card (which carries the "定位" reveal action), not vanish.
    const msgs: HistoryMessage[] = [
      { role: "user", parts: [{ type: "text", text: "修订word文档" }] },
      {
        role: "assistant",
        parts: [
          { type: "text", text: "done" },
          {
            ...
            state: {
              status: "completed",
              input: { path: "test_revised.docx", display: "panel" },
            },
          },
        ],
      },
    ...
    const t = historyToThread(msgs);
    const card = t.blocks.find(
      (b) => b.kind === "artifact" && (b as { path?: string }).path === "test_revised.docx",
    );
    expect(card).toBeTruthy();
    expect((card as { presentation?: { mode?: string } }).presentation?.mode).toBe("panel");
  });
