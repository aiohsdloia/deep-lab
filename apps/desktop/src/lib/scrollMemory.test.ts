...
describe("useChatScroll", () => {
  ...
  it("reopening a chat left at the latest re-snaps to the CURRENT bottom", () => {
    // Jump to latest at height 1000 → remembered at the bottom (800).
    const el = document.createElement("div");
    Object.defineProperties(el, {
      scrollHeight: { value: 1_000, configurable: true },
      clientHeight: { value: 200, configurable: true },
    ...
    // The conversation grew while away (2000 tall) — reopen lands at the NEW
    // bottom, not the stale saved number.
    ...
    renderHook(() => {
      const ref = useRef<HTMLElement | null>(el2);
      return useChatScroll(ref, "chat:ses_1");
    });
    // jsdom does not clamp scrollTop, so "the bottom" here is scrollHeight
    // (the 2_000 value), not the stale 800 the first visit remembered.
    expect(el2.scrollTop).toBe(2_000);
  });

  it("reopening a chat left at an older position stays there", () => {
    // Scrolled up (offset 300) then closed — not "at the latest".
    const el = document.createElement("div");
    Object.defineProperties(el, {
      scrollHeight: { value: 1_000, configurable: true },
      clientHeight: { value: 200, configurable: true },
    ...
    const hook = renderHook(() => {
      const ref = useRef<HTMLElement | null>(el);
      return useChatScroll(ref, "chat:ses_2");
    });
    act(() => {
      el.scrollTop = 300;
      hook.result.current.onScroll({ currentTarget: el } as unknown as UIEvent<HTMLElement>);
    ...
    // Reopen restores the exact position — never yanked to the bottom.
    const el2 = document.createElement("div");
    Object.defineProperties(el2, {
      scrollHeight: { value: 1_000, configurable: true },
      clientHeight: { value: 200, configurable: true },
    ...
    expect(again.result.current.atLatest).toBe(false);
  });
});
