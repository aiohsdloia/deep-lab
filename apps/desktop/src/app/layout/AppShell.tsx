...
import { ensureJupyter, openExternal, setWindowTitle, watchFullscreen } from "@/lib/tauri";
...
export function AppShell() {
  ...
  useEffect(() => {
    ...
    void ensureJupyter();
    // Main window only: add File → Export/Import Settings to the native menu.
    if (!isDetachedScreen) void installAppMenu();
  ...
  // A detached screen window owns its native titlebar — keep its title in sync
  // with the focused session (it has no in-window tab strip to name it).
  useEffect(() => {
    if (!isDetachedScreen) return;
    const syncTitle = () => {
      const layout = useLayoutStore.getState();
      const g = layout.groups.find((x) => x.id === layout.activeGroupId);
      const leaf = g?.tree ? leaves(g.tree).find((l) => l.sessionId) : undefined;
      const sid = leaf?.sessionId ?? null;
      const session = sid ? useRuntimeStore.getState().sessions.find((s) => s.id === sid) : undefined;
      const title = session?.title || sid || g?.name || "DeepLab";
      void setWindowTitle(title);
    };
    syncTitle();
    const unsubLayout = useLayoutStore.subscribe(syncTitle);
    const unsubRuntime = useRuntimeStore.subscribe(syncTitle);
    return () => {
      unsubLayout();
      unsubRuntime();
    };
