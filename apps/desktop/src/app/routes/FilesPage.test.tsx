...
describe("FilesPage", () => {
  ...
  it("lists workspace entries with sizes and opens a file in the previewer", async () => {
    render(<FilesPage />, { wrapper: MemoryRouter });
  ...
  it("restores the last open directory when the global Files page is reopened", async () => {
    const first = render(<FilesPage />, { wrapper: MemoryRouter });
    await userEvent.click(await screen.findByText("data"));
    expect(await screen.findByText("genes.bed")).toBeInTheDocument();
    first.unmount();
  ...
  it("reveals a revealPath: navigates to the folder and flashes the file's row", async () => {
    const consumed = vi.fn();
    render(
      <SessionFilesPane
        sessionId="ses_1"
        sessionDir="/workspace/session-1"
        revealPath="data/genes.bed"
        onRevealConsumed={consumed}
        onClose={() => {}}
      />,
    );
    // The pane navigates into the file's folder, keeps the list visible, and
    // flashes the row (a reveal must not swap the list for the preview).
    expect(await screen.findByText("genes.bed")).toBeInTheDocument();
    const row = screen.getByText("genes.bed").closest("button")!;
