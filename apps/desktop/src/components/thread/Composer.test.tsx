...
describe("Composer", () => {
  it("persists the typed draft per draftKey across unmount/remount (page navigation)", () => {
    useUiStore.setState({ composerDrafts: {} });
    window.localStorage.removeItem("ai4s.composer.drafts.v1");
    const first = render(<Composer onSend={vi.fn()} draftKey="ses_1" />);
    const input = screen.getByLabelText<HTMLTextAreaElement>("Ask anything");
    fireEvent.change(input, { target: { value: "half-typed prompt" } });
    expect(useUiStore.getState().composerDrafts["ses_1"]).toBe("half-typed prompt");
    first.unmount();

    // Navigating back to the session restores what was typed.
    render(<Composer onSend={vi.fn()} draftKey="ses_1" />);
    expect(screen.getByLabelText<HTMLTextAreaElement>("Ask anything").value).toBe(
      "half-typed prompt",
    );

    // Sending clears the draft, so returning again starts empty.
    fireEvent.keyDown(screen.getByLabelText("Ask anything"), { key: "Enter" });
    expect(useUiStore.getState().composerDrafts["ses_1"]).toBeUndefined();
  });
...
describe("Composer prompt queue", () => {
  it("submits the current input to the queue via the paper-plane button", () => {
    const onEnqueue = vi.fn();
    const onSend = vi.fn();
    render(<Composer onSend={onSend} onEnqueue={onEnqueue} />);
    const input = screen.getByLabelText<HTMLTextAreaElement>("Ask anything");
    fireEvent.change(input, { target: { value: "next step" } });
    fireEvent.click(screen.getByLabelText("Add to queue"));
    expect(onEnqueue).toHaveBeenCalledWith("next step");
    expect(onSend).not.toHaveBeenCalled();
    expect(input.value).toBe(""); // cleared for the next input
  });

  it("withholds queueing for a '!' shell input", () => {
    const onEnqueue = vi.fn();
    render(<Composer onSend={vi.fn()} onRunShell={vi.fn()} onEnqueue={onEnqueue} />);
    const input = screen.getByLabelText("Ask anything");
    fireEvent.change(input, { target: { value: "!pwd" } });
    expect(screen.getByLabelText("Add to queue")).toBeDisabled();
  });

  it("Enter while the agent works enqueues the input instead of sending", () => {
    const onSend = vi.fn();
    const onEnqueue = vi.fn();
    render(<Composer onSend={onSend} onEnqueue={onEnqueue} disabled working />);
    const input = screen.getByLabelText<HTMLTextAreaElement>("Ask anything");
    fireEvent.change(input, { target: { value: "next task" } });
    fireEvent.keyDown(input, { key: "Enter" });
    expect(onEnqueue).toHaveBeenCalledWith("next task");
    expect(onSend).not.toHaveBeenCalled();
    expect(input.value).toBe(""); // cleared, ready for the next queued input
  });

  it("shows a queue-count badge and opens the queue editor", () => {
    const onOpenQueue = vi.fn();
    render(<Composer onSend={vi.fn()} onOpenQueue={onOpenQueue} queueCount={3} />);
    expect(screen.getByText("3")).toBeInTheDocument();
    fireEvent.click(screen.getByLabelText("View queue"));
