...
import { Check, ChevronDown, Copy, Loader2, Paperclip, Pencil, RotateCcw } from "lucide-react";
...
export const UserMessage = memo(function UserMessage({
  ...
  const confirmDialog = confirm &&
    // Rendered into <body>: the virtualized rows each carry a `transform`
    // (translateY), which would make this row the containing block for the
    // dialog's `fixed inset-0` overlay and let later rows paint over it.
    createPortal(
      <ConfirmDialog
...
/** Answers longer than this (≈ a few hundred lines of markdown) clamp behind a
 *  "Show more" toggle — only extreme messages clamp, normal ones are untouched. */
const AGENT_CLAMP_CHARS = 12000;
...
export const AgentMessage = memo(function AgentMessage({
  ...
  // Extremely long answers (a wall of markdown) are clamped behind a "Show
  // more" toggle so a single giant block never dominates layout/compositing —
  // only messages beyond the threshold clamp (the normal case is untouched).
  const [clamped, setClamped] = useState(false);
  const long = shown.length > AGENT_CLAMP_CHARS;
  ...
  return (
    ...
    <div className="group" data-agent-message>
      <div className="relative">
        <div className={cn("overflow-hidden", long && !clamped && "max-h-[420px]")}>
      ...
      {long && (
        <button
          ...
          className="mt-1 flex items-center gap-1 rounded-input px-1 py-0.5 text-xs font-medium text-accent transition-colors hover:bg-surface-2"
        >
          <ChevronDown size={13} className={cn("transition-transform duration-200", clamped && "rotate-180")} />
          {clamped ? t("thread.showLess") : t("thread.showMore")}
        </button>
      )}
      {refs.length > 0 && (
