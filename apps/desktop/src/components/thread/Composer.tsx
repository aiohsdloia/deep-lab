...
export function Composer({
  ...
  onInteract?: () => void;
  /** Current queue length for this session — shows a badge on the queue view
   *  button; omitted (no `onOpenQueue`) hides it. */
  queueCount?: number;
  /** "Add to queue": enqueue the current input as a prompt instead of sending
   *  it now. Provided only for a live session with a queue. */
  onEnqueue?: (text: string) => void;
  /** "View queue": open the session's queue editor subpage. */
  onOpenQueue?: () => void;
}) {
  ...
  const [value, setValue] = useState(
    () => restored?.text ?? (draftKey ? (useUiStore.getState().composerDrafts[draftKey] ?? "") : ""),
  );
  ...
  // The draft is bound to this composer's key, so switching sessions / returning
  // from another page restores what was typed (see `draftKey`).
  useEffect(() => {
    if (!draftKey) return;
    setValue(useUiStore.getState().composerDrafts[draftKey] ?? "");
  }, [draftKey]);
  // Mirror every keystroke into the persisted draft; an empty value (send, clear,
  // /command pick) clears it.
  useEffect(() => {
    if (!draftKey) return;
    useUiStore.getState().setComposerDraftFor(draftKey, value);
  }, [value, draftKey]);
  ...
  const onKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      if (working) {
        // While the agent works the send button is Stop and submit() is a
        // no-op — Enter instead queues the input to run after this turn.
        enqueue();
      } else {
        submit();
      }
  ...
  // The queue only runs plain prompts (never "!" shell or a chipped
  // "/command"), and queueing stays live while the agent works — that is
  // exactly when you queue the next prompt to run after this turn.
  const canEnqueue = !!onEnqueue && !!value.trim() && !shellMode && !command;
  const enqueue = () => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-1.5 pt-1">
        ...
        {/* View the session's prompt queue (opens the queue editor subpage). */}
        {onOpenQueue && (
          <button
            className="relative flex h-7 w-7 shrink-0 items-center justify-center rounded-input text-muted hover:bg-surface-2 hover:text-text"
            aria-label={t("composer.queue.viewAria")}
            title={t("composer.queue.viewTitle")}
            onClick={onOpenQueue}
          >
            <ListChecks size={15} />
            {(queueCount ?? 0) > 0 && (
              <span className="absolute -right-0.5 -top-0.5 grid h-3.5 min-w-3.5 place-items-center rounded-full bg-accent px-0.5 text-[9px] font-semibold leading-none text-accent-fg">
                {queueCount}
              </span>
            )}
          </button>
        ...
        <div className="ml-auto flex min-w-0 items-center gap-1.5">
          ...
          {onEnqueue && (
            <button
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-input border border-border text-muted transition-colors hover:bg-surface-2 hover:text-text disabled:opacity-40"
              aria-label={t("composer.queue.enqueueAria")}
              title={t("composer.queue.enqueueTitle")}
              onClick={enqueue}
              disabled={!canEnqueue}
            >
              <Send size={14} />
