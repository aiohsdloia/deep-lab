...
import type { FilePreviewInspector as FilePreviewInspectorT } from "@deeplab/shared";
...
describe("FilePreviewInspector — locate", () => {
  it("shows a 定位 button that fires onLocate when provided", async () => {
    const onLocate = vi.fn();
    render(<FilePreviewInspector data={md} onClose={() => {}} onLocate={onLocate} />);
    await userEvent.click(await screen.findByLabelText("Locate in file browser"));
    expect(onLocate).toHaveBeenCalledTimes(1);
  });

  it("hides the 定位 button without onLocate", () => {
    render(<FilePreviewInspector data={md} onClose={() => {}} />);
