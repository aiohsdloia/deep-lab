...
describe("Sidebar sessions, away from the session route", () => {
  it("clicking a session switches the current Screen to it, no new Screen", async () => {
    renderAt("/skills");
    await userEvent.click(await screen.findByText("older question"));

    // From Skills/Runs/Files the layout change alone is invisible — those
    // routes render instead of the panes, so the click must navigate too.
    expect(navigateSpy).toHaveBeenCalledWith("/live/B");
    const layout = useLayoutStore.getState();
    // The current screen's focused pane now shows B — the click switched the
    // view to that session; it did NOT stack a second Screen beside it.
  ...
  it("New opens its own Screen instead of replacing the pane in use", async () => {
    renderAt("/skills");
    await userEvent.click(await screen.findByRole("button", { name: "New work screen" }));
