...
describe("Sidebar projects", () => {
  ...
  it("colors the folder icon when a project color is set", async () => {
    useRuntimeStore.setState({
      projects: [{ ...PROJECT, color: "blue" }],
    });
    renderAt("/files");
    await screen.findByText("BCI Trends");
    // One of the folder icons picks up the blue accent (#4a86e8) inline.
    const blue = document
      .querySelectorAll("svg")
      .length
      ? Array.from(document.querySelectorAll("svg")).find(
          (s) => s.getAttribute("style")?.includes("rgb(74, 134, 232)"),
        )
      : undefined;
    expect(blue).toBeTruthy();
  });

  it("shows the pin button for a pinned project (accent) and hides it until hover otherwise", async () => {
    useRuntimeStore.setState({
      projects: [{ ...PROJECT, id: "pin", name: "Pinned Lab", pinned: true, color: "red" }],
    });
    renderAt("/files");
    await screen.findByText("Pinned Lab");
