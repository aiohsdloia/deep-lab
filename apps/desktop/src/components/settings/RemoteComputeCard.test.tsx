...
describe("RemoteComputeCard", () => {
  ...
  it("lists a non-Slurm machine with capability chips and never reads the queue", async () => {
    bridge.computeMachines.mockResolvedValue([{ host: "home-3090", label: "8x3090", caps: null, workdir: null, os: null }]);
  ...
  it("adds a machine then probes it via the post-add machine list refetch", async () => {
    ...
    bridge.computeMachines.mockResolvedValueOnce([{ host: "home-3090", label: null, caps: null, workdir: null, os: null }]); // post-add
  ...
  it("shows the Slurm queue for a Slurm machine", async () => {
    bridge.computeMachines.mockResolvedValue([{ host: "login-a", label: null, caps: null, workdir: null, os: null }]);
  ...
  it("offers a sign-in when the machine asks for credentials, and re-probes once signed in", async () => {
    ...
    bridge.computeMachines.mockResolvedValue([{ host: "hpc-login", label: null, caps: null, workdir: null, os: null }]);
