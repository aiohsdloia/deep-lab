...
import { dshAcpLauncher, isTauri } from "@/lib/tauri";
...
export function AcpAgentsCard() {
  ...
  /** Resolve a one-click preset into a concrete agent. DeepSeek Harness ships a
   *  bundled runtime (runtime/dsh-acp), so its preset must point at that
   *  launcher instead of a bare `npx` invocation that cannot find the dsh
   *  config in the workspace folder. */
  const resolvePreset = async (
    preset: Omit<AcpAgentConfig, "id">,
  ): Promise<Omit<AcpAgentConfig, "id">> => {
    if (preset.name !== "DeepSeek Harness") {
      return { ...preset, args: [...preset.args] };
    }
    const launcher = await dshAcpLauncher();
    if (launcher) {
      return { name: preset.name, command: "node", args: [launcher] };
    }
    // Runtime not fetched (dev without fetch-dsh-acp.sh): keep the npx form so
    // the card still shows a working entry the user can edit or investigate.
    return { ...preset, args: [...preset.args] };
  };
      ...
      <div className="divide-y divide-faint">
        ...
        {adding ? (
          ...
          <div className="px-4 py-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {ACP_PRESETS.filter((p) => !agents.some((a) => a.name === p.name)).map((preset) => (
                <button
                  ...
                  onClick={() => {
                    void resolvePreset(preset).then((agent) =>
                      save({ ...agent, id: newAcpAgentId(agents) }),
                    );
                  }}
                >
                  <Plus size={13} />
            ...
            {runtimeKind === "acp" && !running && error && (
              <div className="mt-2 flex items-start gap-2">
                <p className="min-w-0 flex-1 select-text break-all whitespace-pre-wrap font-mono text-xs text-error">
                  {error}
                </p>
                <button
                  type="button"
                  className={cn(iconBtn("shrink-0"), "mt-0.5")}
                  aria-label={t("acp.copyError")}
                  title={t("acp.copyError")}
                  onClick={() => {
                    void navigator.clipboard
                      .writeText(error)
                      .then(() => toast.success(t("acp.copiedError")))
                      .catch(() => {
                        ...
                        ta.remove();
                        toast.success(t("acp.copiedError"));
                      });
                  }}
                >
                  <Copy size={12} />
            ...
            {runtimeKind === "acp" && (running || (!error && status === "connecting")) && (
              <p className="mt-2 text-xs text-muted">
                {running ? t("acp.running") : t("acp.starting")}
              </p>
            )}
          </div>
        )}
