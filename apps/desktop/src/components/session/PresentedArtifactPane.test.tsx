...
describe("PresentedArtifactPane", () => {
  it("shows a 定位 button that reveals the file in the session's browser", async () => {
    const revealFile = vi.fn();
    useRuntimeStore.setState({ revealFile: revealFile as never });
    render(
    ...
    await userEvent.click(screen.getByLabelText("Locate in file browser"));
    expect(revealFile).toHaveBeenCalledWith("session-a", "notes/result.md");
  });
