...
export function SessionView({
  ...
  const clearReveal = useRuntimeStore((s) => s.clearReveal);
  ...
  // Question-history: every user-written message in this session, so the user
  // can search it and jump back to the exact spot. Works with the virtualized
  // list via a scroll handle BlockList fills in.
  const scrollToBlockRef = useRef<((blockIndex: number) => void) | null>(null);
  const [qHistoryOpen, setQHistoryOpen] = useState(false);
  const [qSearch, setQSearch] = useState("");
  const [hlBlock, setHlBlock] = useState<number | null>(null);
  const hlTimer = useRef<number | null>(null);
  const userQuestions = useMemo(
    () =>
      (thread?.blocks ?? [])
        .map((b, i) => ({ i, text: b.kind === "user" ? b.text : null }))
        .filter((q): q is { i: number; text: string } => q.text !== null),
    [thread],
  );
  const qFiltered = useMemo(() => {
    const q = qSearch.trim().toLowerCase();
    return q ? userQuestions.filter((m) => m.text.toLowerCase().includes(q)) : userQuestions;
  }, [userQuestions, qSearch]);

  const jumpToQuestion = (blockIndex: number) => {
    scrollToBlockRef.current?.(blockIndex);
    setHlBlock(blockIndex);
    if (hlTimer.current) window.clearTimeout(hlTimer.current);
    hlTimer.current = window.setTimeout(() => setHlBlock(null), 2200);
  };
  ...
  // The queue editor subpage (opened via the composer's "view queue" button).
  const [queueOpen, setQueueOpen] = useState(false);
  const enqueuePrompt = useRuntimeStore((s) => s.enqueuePrompt);
  const queueCount = useRuntimeStore((s) => (eid ? (s.queues[eid]?.length ?? 0) : 0));
  ...
  const inspectorNode = activeArtifact ? (
    <InspectorShell
      ...
      onLocate={sid ? () => revealFile(sid, activeArtifact.path) : undefined}
    ...
    <SessionFilesPane
      ...
      revealPath={pane?.revealPath}
      onRevealConsumed={() => clearReveal(sid ?? undefined)}
  ...
  return (
    <div className="relative flex h-full min-w-0">
      ...
      <div className="relative flex h-full min-w-0 flex-1 flex-col">
        ...
        >
          ...
          {/* Every question the user asked in THIS session, searchable — click
              one to jump the conversation to that message. */}
          {eid && userQuestions.length > 0 && (
            <Popover.Root open={qHistoryOpen} onOpenChange={setQHistoryOpen}>
              <Popover.Trigger asChild>
                <button
                  className="rounded-md p-1 text-muted transition-colors hover:bg-surface-2 hover:text-text"
                  title={t("questionHistory.open")}
                  aria-label={t("questionHistory.open")}
                >
              ...
              <Popover.Portal>
                <Popover.Content
                  align="end"
                  sideOffset={6}
                  className="z-50 flex max-h-[min(60vh,420px)] w-[min(340px,80vw)] flex-col overflow-hidden rounded-card border border-border bg-surface shadow-pop"
                >
                  <div className="flex items-center gap-2 border-b border-border px-3 py-2">
                    ...
                    <input
                      ...
                      placeholder={t("questionHistory.searchPlaceholder")}
                  ...
                  <div className="overflow-y-auto p-1">
                    {qFiltered.length === 0 ? (
                      <div className="px-3 py-6 text-center text-xs text-muted">
                        {t("questionHistory.empty")}
                      </div>
                    ) : (
                      qFiltered.map(({ i, text }) => (
                        <button
                          key={i}
                          onClick={() => {
                            setQHistoryOpen(false);
                            jumpToQuestion(i);
                          }}
                          className="flex w-full items-start gap-2 rounded-input px-3 py-2 text-left text-xs text-text transition-colors hover:bg-surface-2"
                        >
                          <History size={12} className="mt-0.5 shrink-0 text-muted" />
          ...
          {/* Open this session's folder in the default terminal (default) or VS
              Code — desktop only (needs a local shell/editor). Sits just left of
              the Files toggle so the folder lives in one place. */}
          {eid && sessionDir && isTauri && (
            <DropdownMenu.Root>
              <DropdownMenu.Trigger asChild>
                <button
                  className="rounded-md p-1 text-muted transition-colors hover:bg-surface-2 hover:text-text"
                  title={t("live.openFolder.title")}
                  aria-label={t("live.openFolder.title")}
                >
              ...
              <DropdownMenu.Portal>
                <DropdownMenu.Content
                  align="end"
                  sideOffset={4}
                  className="z-50 min-w-[190px] rounded-card border border-border bg-surface p-1 text-xs text-text shadow-pop"
                >
                  <DropdownMenu.Item
                    onSelect={() => {
                      pinEphemeral();
                      // eslint-disable-next-line i18next/no-literal-string -- FolderOpenTarget enum, not UI copy
                      void openFolderIn(sessionDir, "terminal");
                    }}
                  ...
                  >
                    ...
                    <span>{t("live.openFolder.terminal")}</span>
                  </DropdownMenu.Item>
                  <DropdownMenu.Item
                    onSelect={() => {
                      pinEphemeral();
                      // eslint-disable-next-line i18next/no-literal-string -- FolderOpenTarget enum, not UI copy
                      void openFolderIn(sessionDir, "vscode");
                    }}
                    className="flex cursor-pointer items-center gap-2 rounded-input px-2 py-1.5 outline-none data-[highlighted]:bg-surface-2"
                  >
                    <Code2 size={13} className="shrink-0 text-muted" />
                    <span>{t("live.openFolder.vscode")}</span>
                  </DropdownMenu.Item>
                </DropdownMenu.Content>
              </DropdownMenu.Portal>
            </DropdownMenu.Root>
          ...
          >
            ...
            {connected && isEmpty && !eid && !webReadOnly && (
              <WorkflowStarters
                ...
                // The toast is the visible feedback (the pane is already blank).
                onBlank={() => {
                  useRuntimeStore.setState((s) => {
                    const threads = { ...s.threads };
                    threads[draftKey] = { blocks: [], index: {}, loaded: true };
                    return { currentId: null, threads };
                  });
                  useRuntimeStore.getState().markClean(draftKey);
                  toast.success(t("clean.entered"));
                }}
              />
            )}
            ...
            {!historyLoading && thread && (
              <BlockList
                ...
                workspaceDirectory={sessionDir ?? undefined}
                scrollElementRef={chatRef}
                zoom={zoom}
                scrollToBlockRef={scrollToBlockRef}
                highlightBlockIndex={hlBlock ?? undefined}
              />
            )}
        ...
        >
          ...
          >
            ...
            <Composer
              ...
              onEnqueue={eid ? (text) => enqueuePrompt(eid, text) : undefined}
              onOpenQueue={
                eid
                  ? () => {
                      pinEphemeral();
      ...
      {/* Queue editor subpage — covers this pane (not the whole window). */}
      {eid && queueOpen && <QueuePanel sessionId={eid} onClose={() => setQueueOpen(false)} />}
    </div>
  );
}
