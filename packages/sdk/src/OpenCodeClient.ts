...
const ARTIFACT_PRESENTATION_SYSTEM = `Open Lab can display workspace files through the present_artifact tool.
    ...
    const res = await this.fetchWithTimeout(
      `${this.baseUrl}/session/${encodeURIComponent(sessionId)}/prompt_async`,
      {
        ...
        body: JSON.stringify({
          ...
          ...(agent ? { agent } : {}),
          ...(m ? { model: m } : {}),
          // A "clean" session sends NO preset system template — the plainest,
          // most token-efficient chat possible. Everything else carries the
          // artifact-presentation system message.
          ...(clean ? {} : { system: ARTIFACT_PRESENTATION_SYSTEM }),
