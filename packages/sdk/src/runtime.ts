...
export interface AgentRuntime {
   ...
   *  for the model's default effort. `clean` omits the preset system template
   *  (artifact presentation) — a minimal, token-efficient plain chat. `files`
   *  sends attachments as real multimodal parts so a vision model sees the
   *  image, not just its name (#88). See lib/runtime.ts. */
  sendPrompt(
    sessionId: string,
    text: string,
    agent?: string,
