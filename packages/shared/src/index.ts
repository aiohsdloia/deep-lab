...
/** One recorded write of an artifact — a line in `.openlab/provenance.jsonl`.
...
export interface ProvenanceEnv {
  ...
  platform: string;
  /** Open Lab app version that recorded it. */
  app: string;
 ...
 * compared. Stored append-only in `.openlab/runs.jsonl`.
 */
...
// bundled `openlab.mplstyle` which carries the same hexes). Validated with
