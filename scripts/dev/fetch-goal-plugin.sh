...
# Install runtime deps next to the entry so esbuild can resolve them. The goal
# plugin declares `@opencode-ai/plugin: ^1.17.1`, which resolves to the newest
# release — and the newest plugin depends on a not-yet-published SDK
# (`@opencode-ai/sdk@1.18.12` → ETARGET). Pin the direct dependency to the
# bundled OpenCode version so the whole tree stays consistent with the sidecar.
(
  cd "$TMP/package"
  node -e "const fs=require('fs');const f='package.json';const p=JSON.parse(fs.readFileSync(f));p.dependencies['@opencode-ai/plugin']='${OPENCODE_PLUGIN_VERSION}';fs.writeFileSync(f,JSON.stringify(p,null,2))"
  npm install --silent --no-fund --no-audit --omit=dev --ignore-scripts effect zod > /dev/null
)
