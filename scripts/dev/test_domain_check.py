...
class Phylo(unittest.TestCase):
    def test_euclidean_distance_on_sequences(self):
        fs = findings("import numpy as np\nx = np.sqrt(np.sum((seqs1 - seqs2) ** 2))\n")
        self.assertTrue(any(f.tag == "phylo · distance" for f in fs))

    def test_pdist_on_sequences(self):
        fs = findings("from scipy.spatial.distance import pdist\nd = pdist(sequences, metric='euclidean')\n")
        self.assertTrue(any(f.tag == "phylo · distance" for f in fs))

    def test_aligned_distance_is_ok(self):
        # An explicit alignment / substitution-model context stays silent.
        self.assertNotIn(
            "phylo · distance",
            tags("from Bio import Align\nfrom Bio.Align import substitution_matrices\nm = substitution_matrices.load('BLOSUM62')\n"),
        )

    def test_tree_written_without_support(self):
        src = "from Bio import Phylo\nresult = infer_ml_tree(seqs)\nPhylo.write(result, 'tree.nwk', 'newick')\n"
        fs = findings(src)
        self.assertTrue(any(f.tag == "phylo · support" for f in fs))

    def test_bootstrapped_tree_is_ok(self):
        src = "from Bio import Phylo\nresult = infer_ml_tree(seqs)\nboots = bootstrap(seqs, 100)\nPhylo.write(result, 'tree.nwk', 'newick')\n"
        self.assertNotIn("phylo · support", tags(src))

    def test_no_inference_is_ok(self):
        self.assertNotIn("phylo · support", tags("from Bio import Phylo\nPhylo.read('t.nwk', 'newick')\n"))


class Ecology(unittest.TestCase):
    def test_mean_of_categorical_site_code(self):
        fs = findings("import numpy as np\nm = np.mean(site_id)\n")
        self.assertTrue(any(f.tag == "ecology · categorical" for f in fs))

    def test_plot_code_median(self):
        fs = findings("import statistics\nm = statistics.median(plot)\n")
        self.assertTrue(any(f.tag == "ecology · categorical" for f in fs))

    def test_euclidean_on_community_matrix(self):
        fs = findings("from scipy.spatial.distance import pdist\nd = pdist(comm_matrix)\n")
        self.assertTrue(any(f.tag == "ecology · composition" for f in fs))

    def test_bray_curtis_is_ok(self):
        self.assertNotIn(
            "ecology · composition",
            tags("import skbio\nd = skbio.stats.distance.braycurtis(abundance)\n"),
        )

    def test_continuous_abundance_mean_is_ok(self):
        self.assertNotIn("ecology · categorical", tags("m = np.mean(biomass)\n"))


class Evolution(unittest.TestCase):
    def test_unguarded_jukes_cantor(self):
        src = "import numpy as np\nd = -0.75 * np.log(1 - 4 * p / 3)\n"
        fs = findings(src)
        self.assertTrue(any(f.tag == "evolution · jukes-cantor" for f in fs))

    def test_clipped_jukes_cantor_is_ok(self):
        src = "import numpy as np\np = np.clip(p, 0, 0.749)\nd = -0.75 * np.log(1 - 4 * p / 3)\n"
        self.assertNotIn("evolution · jukes-cantor", tags(src))

    def test_naive_p_distance_as_rate(self):
        fs = findings("dist = n_diff / n_sites\n")
        self.assertTrue(any(f.tag == "evolution · rate" for f in fs))

    def test_corrected_distance_is_ok(self):
        self.assertNotIn(
            "evolution · rate",
            tags("from Bio.Phylo.TreeConstruction import DistanceCalculator\ncalc = DistanceCalculator('model')\n"),
        )
